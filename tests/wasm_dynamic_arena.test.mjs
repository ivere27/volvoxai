...
function dynamicLogicalLinearSnapshot(weightName = 'weight') {
  const definitions = [
    {
      name: weightName, dtype: 'float32', shape: [3, 2],
  ...
  const graph = parseGraphDocument({
    ...
    nodes: [{
      id: 'linear',
      opType: 'Linear',
      inputs: { input: 'input', weight: weightName, bias: 'bias' },
...
function dynamicStructuralSnapshot() {
  const graph = parseGraphDocument({
    format: 'volvox-graph/v1',
    dimensions: {
      B: { min: 1, max: 4 },
...
function structuralInputs(batch, sequence, salt) {
  return {
    a: {
      data: Float32Array.from(
  ...
  try {
    ...
    await t.test('provider eagerly slices partial banks and isolates sibling residencies',
      async () => {
        ...
        assert.ok(source);
        const forked = [];
        const originalFork = source.forkWithCompiledInvariants;
        source.forkWithCompiledInvariants = function observedFork(...args) {
          const engine = originalFork.call(this, ...args);
          forked.push(engine);
          return engine;
        };
        ...
        try {
          ...
          assert.equal(forked[0].mem, forked[1].mem,
            'sibling contexts borrow one physical linear-memory root');
          assert.equal(forked[0].inspectMemoryRoot().engineId,
            forked[1].inspectMemoryRoot().engineId);
          ...
          source.forkWithCompiledInvariants = originalFork;
          await first?.close();
          await second?.close();
    ...
    await t.test('provider disposes a fork exactly once when eager preload compilation fails',
      async () => {
        ...
        const originalFork = source.forkWithCompiledInvariants;
        let forked = null;
        let forkedOffset = null;
        let disposeCalls = 0;
        source.forkWithCompiledInvariants = function failingPreloadFork(...args) {
          const engine = originalFork.call(this, ...args);
          forked = engine;
          forkedOffset = engine.inspectArena().mutableArenaOffsetBytes;
        ...
        try {
          ...
          let recoveredEngine;
          source.forkWithCompiledInvariants = function recoveredFork(...args) {
            recoveredEngine = originalFork.call(this, ...args);
            return recoveredEngine;
          };
          recoveredContext = await compiled.createContext();
          assert.equal(recoveredEngine.inspectArena().mutableArenaOffsetBytes, forkedOffset,
            'failed construction must return its complete mutable range to the pool');
        } finally {
          source.forkWithCompiledInvariants = originalFork;
    ...
    await t.test('public provider grows active Linear extents without repacking weights', async () => {
      ...
      try {
        ...
        for (const result of results) {
          assert.equal(result.report.backendReport.invariantF32PackCount, 0);
          assert.equal(result.report.backendReport.sharedInvariantF32PackCount, 1);
          assert.equal(result.report.backendReport.invariantRawWeightCopyCount, 0);
          assert.equal(result.report.backendReport.sharedInvariantRawWeightCopyCount, 2);
    ...
    await t.test('compiled model creates isolated arenas for concurrent shape contexts', async () => {
      ...
      const originalSharedFork = source.forkWithCompiledInvariants;
      const originalPreparePrefix = source.prepareCompiledInvariantPrefix;
      let compiledPrefix;
      source.forkWithCompiledInvariants = function observedSharedFork(...args) {
        const engine = originalSharedFork.call(this, ...args);
        contextEngines.push(engine);
        return engine;
      };
      source.prepareCompiledInvariantPrefix = function observedPreparePrefix(...args) {
        compiledPrefix = originalPreparePrefix.call(this, ...args);
      ...
      const results = [];
      try {
        compiled = await runtime.compile(dynamicLogicalLinearSnapshot('weight:packed:0'), {
          backend: { mode: 'require', backend: 'wasm', operatorFallback: 'forbid' },
        });
        ...
        for (const [index, result] of results.entries()) {
          ...
          assert.equal(result.report.backendReport.invariantRawWeightCopyCount, 0);
          assert.equal(result.report.backendReport.sharedInvariantRawWeightCopyCount, 2);
          assert.equal(result.report.backendReport.sharedInvariantPhysicalAllocationCount, 1);
        ...
        assert.equal(contextEngines.length, 2);
        assert.equal(contextEngines[0].mem, contextEngines[1].mem);
        const firstArena = contextEngines[0].inspectArena();
        const secondArena = contextEngines[1].inspectArena();
        assert.equal(firstArena.sharedInvariant.prefixId, secondArena.sharedInvariant.prefixId);
        assert.equal(firstArena.sharedInvariant.physicalAllocationCount, 1);
        assert.equal(firstArena.sharedInvariant.borrowerCount, 2);
        assert.equal(firstArena.sharedInvariant.rawWeightCopyCount, 2);
        assert.equal(firstArena.sharedInvariant.f32PackCount, 1);
        assert.ok(firstArena.sharedInvariant.rawWeightPhysicalBytes > 0);
        assert.ok(firstArena.sharedInvariant.packedWeightPhysicalBytes > 0);
        assert.equal(
          firstArena.sharedInvariant.rawWeightPhysicalBytes +
            firstArena.sharedInvariant.packedWeightPhysicalBytes,
          firstArena.sharedInvariant.backingLengthBytes,
          'one physical prefix allocation contains the only raw and packed copies',
        );
        const contextGraph = contextEngines[0].graph;
        contextGraph.tensors.set('rogue:weight', { name: 'rogue:weight', isWeight: true });
        try {
          assert.throws(() => compiledPrefix.install(
            contextGraph, new Map(), new Map(), new Map(), new Map(),
          ), /weight inventory disagrees/,
          'a context graph cannot add a weight outside the compiled prefix');
        } finally {
          contextGraph.tensors.delete('rogue:weight');
        }
        assert.throws(() => source._allocateBytes(16, 'retained-source misuse'),
          /root allocator is sealed/);
        assert.throws(() => source._resetAllocator(), /root allocator is sealed/);
        assert.throws(() => source.api.alloc_bytes(16), /root C allocator is sealed/);
        assert.throws(() => source.api.reset_heap(), /root C allocator is sealed/);
        assert.throws(() => source.api.heap_rewind(16), /root C allocator is sealed/);
        assert.equal('instance' in source.wasmModule, false,
          'the retained provider root must not expose a raw allocator-export bypass');
        assert.throws(() => source.dispose(), /cannot close before every compiled prefix/);
        assert.throws(() => source.compile(
          contextEngines[0].graph,
          contextEngines[0].preparedGraph,
        ), /root owns shared compiled\/context regions/);
        await assert.rejects(source.execute({}), /root execution is sealed/);
        assert.ok(firstArena.mutableArenaOffsetBytes + firstArena.mutableArenaMaximumBytes <=
          secondArena.mutableArenaOffsetBytes,
        'context mutable capacities must occupy disjoint bounded ranges');
        const reuseSentinelOffset = firstArena.mutableArenaOffsetBytes +
          firstArena.mutableArenaMaximumBytes - 16;
        new Uint8Array(source.mem.buffer, reuseSentinelOffset, 16).fill(0xa5);
        ...
        const prefixId = survivor.report.backendReport.sharedInvariantPrefixId;
        await largeContext.close();
        largeContext = null;
        reopenedContext = await compiled.createContext();
        const reopenedArena = contextEngines.at(-1).inspectArena();
        assert.equal(reopenedArena.mutableArenaOffsetBytes, firstArena.mutableArenaOffsetBytes,
          'the free-list should reuse the released mutable capacity');
        assert.deepEqual(
          [...new Uint8Array(source.mem.buffer, reuseSentinelOffset, 16)],
          new Array(16).fill(0),
          'a recycled mutable range must restore fresh-memory zero semantics',
        );
        const reopened = await reopenedContext.execute({
          input: { data: smallValues, shape: [1, 3] },
        });
        results.push(reopened);
        assert.equal(reopened.report.backendReport.sharedInvariantPrefixId, prefixId,
          'closing every context must not release the compiled invariant prefix');
        assert.equal(reopened.report.backendReport.sharedInvariantRawWeightCopyCount, 2);
        assert.equal(reopened.report.backendReport.sharedInvariantF32PackCount, 1);
        assert.equal(reopened.report.backendReport.invariantRawWeightCopyCount, 0);
      } finally {
        source.forkWithCompiledInvariants = originalSharedFork;
        source.prepareCompiledInvariantPrefix = originalPreparePrefix;
        await smallContext?.close();
        await largeContext?.close();
    ...
    await t.test('closing one compiled prefix cannot corrupt a sibling compiled model', async () => {
      const source = await WasmEngine.init(wasmPath);
      assert.ok(source);
      const runtime = new Runtime();
      runtime._addProvider('wasm', new WasmBackendProvider(source));
      ...
      try {
        [compiledA, compiledB] = await Promise.all([
          runtime.compile(dynamicLogicalLinearSnapshot('model:a:weight'), {
            backend: { mode: 'require', backend: 'wasm', operatorFallback: 'forbid' },
          }),
          runtime.compile(dynamicLogicalLinearSnapshot('model:b:weight'), {
            backend: { mode: 'require', backend: 'wasm', operatorFallback: 'forbid' },
          }),
        ]);
        contextA = await compiledA.createContext();
        contextB = await compiledB.createContext();
        const input = Float32Array.of(1, 2, 3);
        const first = await contextA.execute({ input: { data: input, shape: [1, 3] } });
        const second = await contextB.execute({ input: { data: input, shape: [1, 3] } });
        results.push(first, second);
        assert.notEqual(first.report.backendReport.sharedInvariantPrefixId,
          second.report.backendReport.sharedInvariantPrefixId,
          'each provider compilation owns an independent prefix lease');
        const [arenaA, arenaB] = contextEngines.map((engine) => engine.inspectArena());
        assert.ok(
          arenaA.sharedInvariant.backingOffsetBytes +
            arenaA.sharedInvariant.backingLengthBytes <=
            arenaB.sharedInvariant.backingOffsetBytes ||
          arenaB.sharedInvariant.backingOffsetBytes +
            arenaB.sharedInvariant.backingLengthBytes <=
            arenaA.sharedInvariant.backingOffsetBytes,
          'compiled prefixes must occupy non-overlapping physical ranges',
        );

        await contextA.close();
        contextA = null;
        await compiledA.close();
        compiledA = null;
        const survivor = await contextB.execute({
          input: { data: Float32Array.of(-2, 4, 0.5), shape: [1, 3] },
        });
        results.push(survivor);
        assertClose(await survivor.output('out').read(),
          referenceLinear(Float32Array.of(-2, 4, 0.5)),
          'sibling compiled prefix after independent close');
        assert.equal(survivor.report.backendReport.sharedInvariantPrefixId,
          second.report.backendReport.sharedInvariantPrefixId);
      } finally {
        source.forkWithCompiledInvariants = originalSharedFork;
        await contextA?.close();
    ...
    await t.test('public dynamic operator waves retain CPU parity across rebinds', async (t) => {
      for (const [label, snapshot, requests, outputNames] of [
        [
          'structural broadcast/Where/reduction/transpose',
          dynamicStructuralSnapshot(),
          [structuralInputs(1, 2, 3), structuralInputs(4, 5, 7), structuralInputs(1, 2, 11)],
          ['mean', 'transposed'],
        ],
        [
          'spatial Conv2D',
