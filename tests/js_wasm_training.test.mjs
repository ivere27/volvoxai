...
function padGraph() {
  ...
  const {out}=graph.addOp('Pad',{input},{out:[1,4,5,2]},{pads:[1,2,1,1],value:-.25}); graph.outputNames=[out.name]; return graph;
}

function interp1dGraph(opType='Interp1D'){
  const graph=new Graph(),input=graph.addWeight('input',[2,1,2],'float32',{buffer:Float32Array.from([0,2,10,14])});
  const {out}=graph.addOp(opType,{input},{out:[2,1,4]},{size:4});graph.outputNames=[out.name];return graph;
}
...
function splitGraph() {
  ...
  graph.outputNames = [outputs.out1.name];
  return graph;
}
...
test('explicit WASM training uses C backward and matches CPU linear SGD and AdamW', {
  ...
  try {
    ...
    for (const updateMode of ['sgd', 'adamw']) {
      ...
      assert.equal(wasmGraph.trainingStep, 1);
      assert.ok(before.some((value, index) => value !== wasmGraph.getTensor('weight').buffer[index]));
      ...
      closeArray(wasmGraph.getTensor('weight').buffer, cpuGraph.getTensor('weight').buffer, `${updateMode} weight`);
      closeArray(wasmGraph.getTensor('bias').buffer, cpuGraph.getTensor('bias').buffer, `${updateMode} bias`);
    }
  } finally {
...
test('WASM training matches CPU for broadcast Add and deterministic Dropout', {
  ...
  try {
    ...
    closeArray(wasmGraph.getTensor('bias').buffer, cpuGraph.getTensor('bias').buffer, 'broadcast/dropout update');
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
  ...
  try {
    ...
    for (const opType of ['Sub', 'Div']) {
      ...
      for (const name of ['left', 'right']) {
        ...
        closeArray(wasmGraph.getTensor(name).buffer, cpuGraph.getTensor(name).buffer, `${opType} ${name} update`);
      }
    }
  } finally {
    await rm(directory, { recursive: true, force: true });
...
test('WASM training matches CPU for Expand forward and broadcast-reduction backward', {
  ...
  try {
    ...
    closeArray(wasmGraph.getTensor('input').buffer, cpuGraph.getTensor('input').buffer, 'Expand input update');
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test('WASM training matches CPU for NHWC GroupNorm', {
  ...
  try {
    ...
    closeArray(wasmGraph.getTensor('weight').buffer, cpuGraph.getTensor('weight').buffer, 'GroupNorm weight update');
    closeArray(wasmGraph.getTensor('bias').buffer, cpuGraph.getTensor('bias').buffer, 'GroupNorm bias update');
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
...
test('WASM training matches CPU for Conv2D weight and bias updates', { timeout: 120_000 }, async (t) => {
  ...
  try {
    ...
    closeArray(wasmGraph.getTensor('weight').buffer, cpuGraph.getTensor('weight').buffer, 'Conv2D weight update');
...
test('WASM training matches CPU for Conv1D input, weight, and bias updates', { timeout: 120_000 }, async (t) => {
  ...
  try {
    ...
    const wasm = await api.trainStep(wasmGraph, { ...options, backend: 'wasm' }); const cpu = await CPUAutograd.trainStep(cpuGraph, options);
    assert.ok(Math.abs(wasm.loss - cpu.loss) < 2e-5);
    for (const name of ['input', 'weight', 'bias']) {
      closeArray(wasm.gradients.get(name), cpu.gradients.get(name), `Conv1D ${name} gradient`);
      closeArray(wasmGraph.getTensor(name).buffer, cpuGraph.getTensor(name).buffer, `Conv1D ${name} update`);
    }
  } finally { await rm(directory, { recursive: true, force: true }); }
});
...
test('WASM training matches CPU for ConvTranspose2D gradients', { timeout: 120_000 }, async (t) => {
  ...
  try {
    const wasmPath=join(directory,'volvoxai.full.wasm'); await buildFullWasm(wasmPath); const api=await TrainingVolvoxAI.init(['wasm'],wasmPath);
    ...
    const wasm=await api.trainStep(wasmGraph,{...options,backend:'wasm'}),cpu=await CPUAutograd.trainStep(cpuGraph,options); assert.ok(Math.abs(wasm.loss-cpu.loss)<2e-5);
    for(const name of ['input','weight','bias']) { closeArray(wasm.gradients.get(name),cpu.gradients.get(name),`ConvTranspose2D ${name} gradient`); closeArray(wasmGraph.getTensor(name).buffer,cpuGraph.getTensor(name).buffer,`ConvTranspose2D ${name} update`); }
  } finally { await rm(directory,{recursive:true,force:true}); }
});
...
test('WASM training matches CPU for constant Pad forward and backward', {timeout:120_000}, async(t)=>{
  ...
  try{const wasmPath=join(directory,'volvoxai.full.wasm');await buildFullWasm(wasmPath);const api=await TrainingVolvoxAI.init(['wasm'],wasmPath),wasmGraph=padGraph(),cpuGraph=padGraph(),options={targets:Array.from({length:20},(_,i)=>i%2),trainableTensors:['input'],updateMode:'sgd',optimizer:{learningRate:.01}};
    const wasm=await api.trainStep(wasmGraph,{...options,backend:'wasm'}),cpu=await CPUAutograd.trainStep(cpuGraph,options);assert.ok(Math.abs(wasm.loss-cpu.loss)<2e-5);closeArray(wasm.gradients.get('input'),cpu.gradients.get('input'),'Pad input gradient');closeArray(wasmGraph.getTensor('input').buffer,cpuGraph.getTensor('input').buffer,'Pad input update');
  }finally{await rm(directory,{recursive:true,force:true});}
});

test('WASM training matches CPU for batched Interp1D gradients',{timeout:120_000},async(t)=>{
  ...
  try{const wasmPath=join(directory,'volvoxai.full.wasm');await buildFullWasm(wasmPath);const api=await TrainingVolvoxAI.init(['wasm'],wasmPath);
    for(const opType of ['Interp1D','InterpLinear1D']){const wasmGraph=interp1dGraph(opType),cpuGraph=interp1dGraph(opType),options={targets:[1,2],trainableTensors:['input'],updateMode:'sgd',optimizer:{learningRate:.01}};const wasm=await api.trainStep(wasmGraph,{...options,backend:'wasm'}),cpu=await CPUAutograd.trainStep(cpuGraph,options);assert.ok(Math.abs(wasm.loss-cpu.loss)<2e-5,opType);closeArray(wasm.gradients.get('input'),cpu.gradients.get('input'),`${opType} input gradient`);closeArray(wasmGraph.getTensor('input').buffer,cpuGraph.getTensor('input').buffer,`${opType} input update`);}
  }finally{await rm(directory,{recursive:true,force:true});}
});
...
test('WASM training matches CPU for fixed-statistics BatchNorm2D', { timeout: 120_000 }, async (t) => {
  ...
  try {
    ...
    closeArray(wasmGraph.getTensor('weight').buffer, cpuGraph.getTensor('weight').buffer, 'BatchNorm2D weight update');
  } finally { await rm(directory, { recursive: true, force: true }); }
});
...
test('WASM training matches CPU for bilinear, nearest, and 2x upsample aliases', { timeout: 120_000 }, async (t) => {
  try { await run(clang, ['--version']); } catch (error) { if (error?.code === 'ENOENT') return t.skip(`${clang} is unavailable`); throw error; }
  const directory = await mkdtemp(join(tmpdir(), 'volvoxai-wasm-resize-'));
  try {
    const wasmPath = join(directory, 'volvoxai.full.wasm'); await buildFullWasm(wasmPath);
    const api = await TrainingVolvoxAI.init(['wasm'], wasmPath);
    for (const opType of ['Resize', 'ResizeNearest2D', 'UpsampleNearest2D', 'Upsample2x']) {
      const wasmGraph = resize2dGraph(opType), cpuGraph = resize2dGraph(opType);
      const options = { targets: Array.from({ length: ['UpsampleNearest2D', 'Upsample2x'].includes(opType) ? 16 : 9 }, (_, index) => index % 2), trainableTensors: ['input'], updateMode: 'sgd', optimizer: { learningRate: 0 } };
      const wasm = await api.trainStep(wasmGraph, { ...options, backend: 'wasm' }); const cpu = await CPUAutograd.trainStep(cpuGraph, options);
      assert.ok(Math.abs(wasm.loss - cpu.loss) < 2e-5, opType);
      closeArray(wasm.gradients.get('input'), cpu.gradients.get('input'), `${opType} input gradient`);
    }
