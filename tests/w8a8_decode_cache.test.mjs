...
import { singleLaneRowSet } from '../ts/backends/decodeRowSet.js';
...
test('mixed-precision row Add fails closed on a non-broadcast operand', () => {
  ...
  assert.throws(() => quantizedRowNode(graph.nodes[0], singleLaneRowSet(1)),
...
test('byte Expand row keeps an invariant [1,1,D] source and slices only output', () => {
  ...
  const row = quantizedRowNode(graph.nodes[0], singleLaneRowSet(2));
...
test('Reshape row accepts only layout-preserving contiguous sequence views', () => {
  ...
  assert.throws(() => quantizedRowNode(invalid.nodes[0], singleLaneRowSet(1)),
    /output must use \[B,3,D\], \[3,1,D\], or \[3,D\] storage/);
});
