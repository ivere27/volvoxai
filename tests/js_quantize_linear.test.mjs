...
test('quantized tensor metadata survives a checkpoint round-trip', () => {
  ...
  const checkpoint = exportModelCheckpoint(snapshot, { includeOptimizerState: false });
  const descriptor = checkpoint.logicalGraph.quantization.tensors.quantized;
  assert.deepEqual(descriptor, {
    scheme: 'per_tensor', scale_tensor: 'scale', zero_point_tensor: 'zero_point',
  });
  assert.deepEqual(checkpoint.logicalGraph.nodes[0].inputs, {
    input: 'input', scale: 'scale', zero_point: 'zero_point',
  });
  assert.equal(JSON.stringify(checkpoint.logicalGraph).includes('"scale":0.25'), false);
  const stored = SafetensorsFile.fromArrayBuffer(checkpoint.parameters);
  ...
  const restored = importModelCheckpoint(checkpoint).snapshot;
  assert.deepEqual(restored.quantizationByTensor.quantized, {
    scheme: 'per_tensor', scale: 0.25, zero_point: 0,
  });
  assert.deepEqual(restored.quantizationByTensor.qweight, {
    scheme: 'per_axis', axis: 0, scales: [0.5, 0.25], zero_points: [0, 0],
  });
  assert.deepEqual([...restored.copyWeightData('qweight')], [1, 2, 3, 4]);
});
