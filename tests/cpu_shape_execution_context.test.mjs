...
test('dynamic CPU slice matches a reference and reuses one capacity pool small -> large -> small', async () => {
  ...
  assert.deepEqual(inspection.invariantWeights,
    { tensorCount: 4, sizeBytes: 204, sharedSizeBytes: 0 },
    'the first context over a revision allocates the weights it reports');
...
/* docs/adr-dynamic-shape-v1.md assigns invariant packed weights to the compiled
 * model, not to a context. TODO.md **Finish exact compiled-artifact ownership
 * of invariant resources** records the remaining common ownership work: N-way
 * concurrency must cost `weights + N × activations`, not
 * `N × (weights + activations)`. */
test('contexts over one revision share invariant weights instead of copying them', async () => {
  const snapshot = transformerSliceSnapshot().snapshot;
  const contexts = [];
  const outputs = [];
  const ids = Int32Array.of(1, 2, 3);

  for (let index = 0; index < 4; index++) {
    const context = new CPUShapeExecutionContext(snapshot);
    const result = await context.execute({ ids: { data: ids, shape: [1, 3] } });
    outputs.push(new Float32Array(result.get('logits').data));
    contexts.push(context);
  }

  const reports = contexts.map((context) => context.inspect().invariantWeights);
  const perContextBytes = reports[0].sizeBytes;
  assert.ok(perContextBytes > 0);

  assert.equal(reports[0].sharedSizeBytes, 0, 'the first context materializes');
  for (const report of reports.slice(1)) {
    assert.equal(report.sizeBytes, perContextBytes, 'every context sees the same weights');
    assert.equal(report.sharedSizeBytes, perContextBytes, 'and none of the later ones paid for them');
  }

  const allocated = reports.reduce(
    (total, report) => total + (report.sizeBytes - report.sharedSizeBytes), 0);
  assert.equal(allocated, perContextBytes,
    `four contexts must allocate the weights once, not ${4 * perContextBytes} bytes`);

  /* Sharing a buffer is only correct if nothing writes to it. Identical inputs
   * through four contexts over one storage must still agree bit for bit. */
  for (const produced of outputs.slice(1)) {
...
test('compiled borrow-only weights survive N context lifetimes and zero-borrow reopen', async () => {
  const snapshot = transformerSliceSnapshot().snapshot;
  const owner = new InvariantResourceStore((value) => value.byteLength);
  let materializations = 0;
  for (const name of snapshot.weightNames) {
    owner.defineLazy(name, () => {
      materializations++;
      return snapshot.copyWeightData(name);
    });
  }
  const ids = Int32Array.of(1, 2, 3);
  const contexts = Array.from({ length: 6 }, () => {
    ...
    return {
      lease,
      context: new CPUShapeExecutionContext(snapshot, { invariantWeightBorrow: lease }),
    };
  });
  const outputs = await Promise.all(contexts.map(({ context }) =>
    context.execute({ ids: { data: ids, shape: [1, 3] } })));
  assert.equal(materializations, snapshot.weightNames.length);
  assert.equal(owner.borrowerCount, 6);
  assert.equal(owner.resourceCount, snapshot.weightNames.length);
  const ownedBytes = owner.ownedBytes;
  assert.ok(ownedBytes > 0);
  for (const { context } of contexts) {
    assert.equal(context.inspect().invariantWeights.sharedSizeBytes, ownedBytes);
  }
  for (const output of outputs.slice(1)) {
    assert.deepEqual([...output.get('logits').data], [...outputs[0].get('logits').data]);
  }

  for (const { context, lease } of contexts) {
    await context.close();
    lease.release();
  }
  assert.equal(owner.borrowerCount, 0);
  assert.equal(owner.ownedBytes, ownedBytes);

  const reopenedLease = owner.open();
  const reopened = new CPUShapeExecutionContext(snapshot, {
    invariantWeightBorrow: reopenedLease,
  });
  await reopened.execute({ ids: { data: ids, shape: [1, 3] } });
  assert.equal(materializations, snapshot.weightNames.length,
    'reopening after every context closed must not copy invariant weights again');
  await reopened.close();
  reopenedLease.release();
  owner.close();
});

test('a derived weight revision does not inherit the previous storage', async () => {
  const base = transformerSliceSnapshot();
  const first = new CPUShapeExecutionContext(base.snapshot);
  await first.execute({ ids: { data: Int32Array.of(1, 2, 3), shape: [1, 3] } });
  const firstLogits = new Float32Array(
    (await first.execute({ ids: { data: Int32Array.of(1, 2, 3), shape: [1, 3] } })).get('logits').data);

  /* A different Model instance is a different revision, so it keys a different
   * entry: new weights must not be served the old bytes. */
  const second = transformerSliceSnapshot();
  const other = new CPUShapeExecutionContext(second.snapshot);
  const report = other.inspect;
  await other.execute({ ids: { data: Int32Array.of(1, 2, 3), shape: [1, 3] } });
  assert.equal(other.inspect().invariantWeights.sharedSizeBytes, 0,
    'a separate revision materializes its own weights');
  assert.ok(report);
  assert.ok(firstLogits.length > 0);
