...
import {
  ...
  DIRECT_OPERATOR_SHAPE_FUNCTIONS,
...
test('direct registry exposes stable IDs, intentional mappings, and fail-closed lookup', () => {
  assert.equal(DIRECT_SHAPE_FUNCTION_IDS.dense, 'volvox.shape.dense-last-axis.v1');
  assert.equal(
    DIRECT_OPERATOR_SHAPE_FUNCTIONS.Linear,
    DIRECT_OPERATOR_SHAPE_FUNCTIONS.Gemm,
  );
  assert.equal(
    DIRECT_OPERATOR_SHAPE_FUNCTIONS.Gemm,
    DIRECT_OPERATOR_SHAPE_FUNCTIONS.MatMul,
  );
  assert.equal(DIRECT_OPERATOR_SHAPE_FUNCTIONS.Add, DIRECT_OPERATOR_SHAPE_FUNCTIONS.Mul);
  assert.equal(DIRECT_OPERATOR_SHAPE_FUNCTIONS.ReLU, DIRECT_OPERATOR_SHAPE_FUNCTIONS.GELU);
  assert.equal(DIRECT_OPERATOR_SHAPE_FUNCTIONS.PReLU, DIRECT_OPERATOR_SHAPE_FUNCTIONS.ReLU);
  assert.notEqual(DIRECT_OPERATOR_SHAPE_FUNCTIONS.Cast, DIRECT_OPERATOR_SHAPE_FUNCTIONS.Identity);
  ...
  assert.equal(Object.isFrozen(DIRECT_OPERATOR_SHAPE_FUNCTIONS), true);

  for (const [operator, shapeFunctionId] of Object.entries(DIRECT_OPERATOR_SHAPE_FUNCTIONS)) {
