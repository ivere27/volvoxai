...
type TokenizerSource = string | URL;

export class Tokenizer {
    vocabByText: Map<string, number>;
    vocabByLatin1: Map<string, number>;
    idToText: string[];
    idToBytes: Uint8Array[];
    merges: Map<string, MergeRule>;
    ...
    async load(vocabUrl: TokenizerSource, mergesUrl: TokenizerSource | null = null): Promise<void> {
        try {
            const isBin = /\.bin$/i.test(String(vocabUrl));
    ...
    async loadFromJson(vocabUrl: TokenizerSource): Promise<void> {
        const res = await fetch(vocabUrl);
        if (!res.ok) throw new Error(`Failed to fetch vocab: ${res.statusText}`);
        const vocabJson = await res.json() as Record<string, number>;
    ...
    async loadFromBinary(vocabUrl: TokenizerSource): Promise<void> {
        const res = await fetch(vocabUrl);
    ...
    async loadMerges(mergesUrl: TokenizerSource): Promise<void> {
    ...
    pairKey(left: number, right: number): string {
        return `${left},${right}`;
    }

    normalizeMergeToken(token: string): string {
        if (token.startsWith("Ġ")) return ` ${token.slice(1)}`;
        return token;
    }

    findTokenByText(text: string): number {
    ...
    findTokenByBytes(bytes: Uint8Array): number {
    ...
    encodeBpeWord(word: string, maxTokens: number): number[] {
    ...
    encodeGreedy(text: string, maxTokens: number): number[] {
        const bytes = this.encoder.encode(text);
        const tokens: number[] = [];
        let pos = 0;
    ...
    // Apply byte-level regex pretokenization and one BPE pass per chunk.
    encode(text: string, maxTokens = 256): number[] {
