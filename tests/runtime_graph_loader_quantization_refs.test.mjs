...
async function load(document, buffer = tensorFile()) {
  ...
  return RuntimeGraphLoader.load(new RuntimeGraph(), ['model.safetensors'], {
    graphUrl: 'graph.json', fetch,
  });
}

test('RuntimeGraphLoader hydrates v1 affine references from safetensors', async () => {
...
test('RuntimeGraphLoader v1 rejects every legacy inline quantization location', async () => {
...
test('RuntimeGraphLoader recursively rejects retired affine params but keeps attention scale', async (t) => {
...
test('RuntimeGraphLoader accepts only volvox-graph/v1', async () => {
...
test('RuntimeGraphLoader rejects obsolete companion metadata in safetensors', async () => {
  const metadata = {
...
test('RuntimeGraphLoader validates referenced parameter dtype, shape, and values', async () => {
  await assert.rejects(
...
test('RuntimeGraphLoader requires Q/DQ operands to use the descriptor references', async () => {
  const document = graphDocument();
...
test('RuntimeGraphLoader prevents quantization parameters from becoming public outputs', async () => {
  const document = graphDocument();
  document.outputs = ['sy'];
  await assert.rejects(load(document), /quantization parameter tensors cannot be public graph outputs/);
});

test('RuntimeGraphLoader prevents quantization parameters from entering graph topology', async () => {
  const input = graphDocument();
