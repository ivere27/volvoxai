...
test('BatchMatMul backward matches finite differences under batch broadcast', async (t) => {
  const cases = [
    { label: 'shared batch', a: [2, 3, 2], b: [2, 2, 3], out: [2, 3, 3], targets: [1, 0, 2, 2, 1, 0] },
    { label: 'broadcast right', a: [3, 2, 2], b: [1, 2, 2], out: [3, 2, 2], targets: [1, 0, 0, 1, 1, 0] },
    { label: 'broadcast left', a: [1, 2, 2], b: [3, 2, 2], out: [3, 2, 2], targets: [0, 1, 1, 0, 0, 1] },
    { label: 'rank four', a: [2, 2, 2, 2], b: [2, 2, 2, 2], out: [2, 2, 2, 2], targets: [1, 0, 0, 1, 1, 1, 0, 0] },
  ];
  const sample = (count, seed) => Array.from({ length: count }, (_, index) =>
    Math.sin((index + 1) * seed) * 0.9);
  const elements = (shape) => shape.reduce((total, value) => total * value, 1);

  for (const spec of cases) {
    await t.test(spec.label, async () => {
      const graph = new Graph();
      const a = addWeight(graph, 'a', spec.a, sample(elements(spec.a), 1.7));
      const b = addWeight(graph, 'b', spec.b, sample(elements(spec.b), 2.3));
      const { out } = graph.addOp('BatchMatMul', { a, b }, { out: spec.out }, {});
      graph.setOutputs([out.name]);
      const { result, engine } = await gradients(graph, ['a', 'b'], spec.targets);
      for (const name of ['a', 'b']) {
        const tensor = graph.getTensor(name);
        for (let index = 0; index < tensor.buffer.length; index++) {
          const numeric = await numericGradient(graph, engine, name, index, spec.targets);
          close(result.gradients.get(name)[index], numeric, `${spec.label} ${name}[${index}]`);
        }
      }
    });
  }
...
test('BatchMatMul backward rejects operands that do not contract', async () => {
  const graph = new Graph();
  const a = addWeight(graph, 'a', [2, 3, 2], new Array(12).fill(0.5));
  const b = addWeight(graph, 'b', [2, 2, 3], new Array(12).fill(0.5));
  const { out } = graph.addOp('BatchMatMul', { a, b }, { out: [2, 3, 3] }, {});
  graph.setOutputs([out.name]);
  b.shape = [2, 3, 3];
  b.buffer = new Float32Array(18).fill(0.5);

  await assert.rejects(() => CPUAutograd.trainStep(graph, {
    targets: [1, 0, 2, 2, 1, 0],
    trainableTensors: ['a', 'b'],
    updateMode: 'sgd',
    optimizer: { learningRate: 0 },
  }), /BatchMatMul/);
});
