...
test('browser profiles stay monolithic and preserve their dependency boundaries', async () => {
  ...
  for (const [profile, inputs] of [
    ['inference', inferenceInputs],
    ['full', Object.keys(full.metafile.inputs)],
    ['wasm', Object.keys(wasm.metafile.inputs)],
  ]) {
    assert.equal(
      inputs.some((path) => path.startsWith('runtime/generated/typescript/') ||
        path.startsWith('runtime/typescript/')),
      false,
      `${profile} bundle unexpectedly contains the Synurang codec or validator`,
    );
    assert.ok(
      inputs.includes('ts/core/MemoryCapture.ts'),
      `${profile} bundle is missing the inference-safe memory capture DTO`,
    );
  }
