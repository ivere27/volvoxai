...
function syntheticRuntimeEngine({
  ...
  const pageCount = memoryByteLength / wasmPageBytes;
  const exports = {
    memory: Number.isInteger(pageCount) && memoryByteLength < 0x80000000
      ? new WebAssembly.Memory({ initial: pageCount })
...
test('WASM memory-root inspection reports exact extent and advances only on backing changes', () => {
  ...
  try {
    const initial = engine.inspectMemoryRoot();
    assert.equal(initial.generation, 1);
    assert.equal(initial.addressableBytes, wasmPageBytes);
    assert.match(initial.engineId, /^wasm-engine-.+/);
    assert.equal(Object.isFrozen(initial), true);
    assert.deepEqual(JSON.parse(JSON.stringify(initial)), initial,
      'the inspection must contain only JSON-safe values');
    assert.deepEqual(engine.inspectMemoryRoot(), initial,
      'an unchanged backing must keep its generation and engine identity');

    memory.grow(1);
    const grown = engine.inspectMemoryRoot();
    assert.deepEqual(grown, {
      engineId: initial.engineId,
      generation: 2,
      addressableBytes: 2 * wasmPageBytes,
    });
    assert.deepEqual(engine.inspectMemoryRoot(), grown,
      'repeated inspection must not advance the generation');

    memory.grow(1);
    assert.deepEqual(engine.inspectMemoryRoot(), {
      engineId: initial.engineId,
      generation: 3,
      addressableBytes: 3 * wasmPageBytes,
    });
  } finally {
    engine.dispose();
  }
...
test('every WASM engine owns a distinct linear-memory root identity', () => {
  const engines = [];
  try {
    for (let index = 0; index < 3; index++) {
      engines.push(new WasmEngine({ instance: { exports: {
        memory: new WebAssembly.Memory({ initial: 1, maximum: 2 }),
        alloc_bytes: () => 16,
        reset_heap: () => {},
      } } }));
    }
    // These direct low-level engines intentionally own independent memories;
    // production provider contexts instead share one pooled root. A bare
    // generation counter would name these separate physical copies alike.
    const identities = engines.map((engine) => engine.inspectMemoryRoot().engineId);
    assert.equal(new Set(identities).size, engines.length);
    assert.deepEqual(
      engines.map((engine) => engine.inspectMemoryRoot().generation),
      [1, 1, 1],
      'distinct engines must not need distinct generations to stay distinct',
    );
  } finally {
...
test('WASM provider exposes one shared root and exact compiled/context suballocations', async () => {
  const source = syntheticRuntimeEngine();
  const provider = new WasmBackendProvider(source);
  const model = identitySnapshot(1);
  let compiled;
  let context;
  let invariantLease;
  try {
    compiled = await provider.compile(createBackendCompileInput(model), {
      operatorFallback: 'forbid',
    });
    assert.equal(compiled.invariantResources.resourceCount, 2,
      'the owner materializes one host aggregate and one linear prefix');
    assert.equal(compiled.invariantResources.ownedBytes, 0,
      'a weightless graph has no physical invariant payload bytes');
    assert.equal(compiled.compilationEvidence.allocationBytes,
      compiled.invariantResources.ownedBytes);
    invariantLease = compiled.invariantResources.open();
    context = await compiled.createContext({
      invariantResources: invariantLease,
      initialPlan: model.staticShapePlan,
    });
    assert.equal(invariantLease.borrowedResourceCount, 2);
    assert.equal(invariantLease.borrowedBytes, compiled.invariantResources.ownedBytes);
    const subject = Object.freeze({
      kind: MemoryOwnerKind.ExecutionContext,
      ownerId: 'context-1',
    });
    const snapshot = context.captureMemorySnapshot({
    ...
    const { engineId } = source.inspectMemoryRoot();
    const root = snapshot.resources.find((resource) =>
      resource.backingRelation === MemoryBackingRelation.Independent);
    const mutable = snapshot.resources.find((resource) =>
      resource.backingRelation === MemoryBackingRelation.Suballocation);
    assert.deepEqual(root, {
      resourceId: `wasm-linear-${engineId}-1`,
      backingResourceId: '',
      backingRelation: MemoryBackingRelation.Independent,
      owner: { kind: MemoryOwnerKind.BackendShared, ownerId: engineId },
      role: MemoryResourceRole.Arena,
      space: MemorySpace.WasmLinear,
      allocator: 'WebAssembly.Memory',
      consumers: [
        { kind: MemoryOwnerKind.CompiledModel, ownerId: root.consumers[0].ownerId },
        subject,
      ],
      measurements: [{
        metric: MemoryMetric.Capacity,
        bytes: { bytes: wasmPageBytes },
      ...
      addressableBytes: { bytes: wasmPageBytes },
    });
    assert.equal(mutable.backingResourceId, root.resourceId);
    assert.equal(mutable.backingRelation, MemoryBackingRelation.Suballocation);
    assert.deepEqual(mutable.owner, subject);
    assert.equal(mutable.role, MemoryResourceRole.Arena);
    assert.ok(mutable.backingLengthBytes.bytes > 0);
    assert.equal(Object.isFrozen(root.measurements[0]), true);
    assert.deepEqual(normalizeBackendMemorySnapshot(snapshot), snapshot,
      'the core provider boundary must accept shared-root suballocation geometry');
  } finally {
    await context?.close();
    invariantLease?.release();
...
test('WASM resource proof includes host weights, exact bank staging, and results', async () => {
  ...
  try {
    ...
    assert.equal(weightedCompiled.invariantResources.resourceCount, 2,
      'host weights and the materialized linear prefix are both owner resources');
    assert.equal(weightedCompiled.invariantResources.ownedBytes, 24,
      '8 host bytes plus one 16-byte-aligned raw linear allocation are owned once');
    assert.equal(weightedCompiled.compilationEvidence.allocationBytes,
      weightedCompiled.invariantResources.ownedBytes,
      'compilation evidence exactly reports every compiled-owned physical byte');
    assert.equal(resources.maximumLinearResidentBytes, initialMemoryBytes);
...
test('WASM host-result ceiling rejects compile and decode ceiling rejects before fork', async () => {
  ...
  const decodeModel = identitySnapshot(decodeBoundaryExtent);
  let compiled;
  let invariantLease;
  try {
    compiled = await decodeProvider.compile(
      createBackendCompileInput(decodeModel),
      { operatorFallback: 'forbid' },
    );
    const resources = compiled.compilationEvidence.shapeDomain;
    ...
    await assert.rejects(
      compiled.createContext({
        invariantResources: invariantLease,
        initialPlan: decodeModel.staticShapePlan,
        decode: {},
      }),
