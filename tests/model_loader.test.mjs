...
/* `dimensionBounds` moves a declared bound at load time.
 *
 * The bound is a deployment property — one package, a phone that wants one row
 * and a server that wants thirty-two — so the caller sets it and the artifact
 * does not have to be re-exported per deployment. It is applied before the
 * document is parsed, which is what makes the fingerprint and the shape domain
 * proof describe the bounds actually in force. */
test('dimensionBounds replaces declared bounds and re-derives the graph identity', async () => {
  const fetchDoc = packageFetch(symbolicDocument(), {
    'weights.safetensors': constantWeights(4),
  });

  const shipped = await ModelLoader.load('weights.safetensors', {
    graphUrl: 'graph.json', fetch: fetchDoc,
  });
  assert.deepEqual(shipped.graph.dimensions.B, { name: 'B', min: 1, max: 8, multiple_of: 1 });

  const widened = await ModelLoader.load('weights.safetensors', {
    graphUrl: 'graph.json', fetch: fetchDoc,
    dimensionBounds: { B: { max: 32 } },
  });
  assert.deepEqual(widened.graph.dimensions.B, { name: 'B', min: 1, max: 32, multiple_of: 1 });
  // Untouched symbols keep every authored field, multiple_of included.
  assert.deepEqual(widened.graph.dimensions.S, { name: 'S', min: 2, max: 32, multiple_of: 2 });

  /* A different domain is a different compiled artifact, so it must not share
   * an identity with the shipped one — a cache keyed on the fingerprint would
   * otherwise hand a B=32 caller the plan built for B=8. */
  assert.notEqual(widened.graph.fingerprint, shipped.graph.fingerprint);

  const narrowed = await ModelLoader.load('weights.safetensors', {
    graphUrl: 'graph.json', fetch: fetchDoc,
    dimensionBounds: { B: { min: 1, max: 1 } },
  });
  assert.deepEqual(narrowed.graph.dimensions.B, { name: 'B', min: 1, max: 1, multiple_of: 1 });
});

test('dimensionBounds refuses a symbol the graph does not declare', async () => {
  await assert.rejects(
    () => ModelLoader.load('weights.safetensors', {
      graphUrl: 'graph.json',
      fetch: packageFetch(symbolicDocument(), { 'weights.safetensors': constantWeights(4) }),
      dimensionBounds: { Nope: { max: 4 } },
    }),
    /names 'Nope', which the graph does not declare\. Declared symbols: B, S/,
  );
});

test('dimensionBounds cannot smuggle an illegal bound past the parser', async () => {
  const fetchDoc = packageFetch(symbolicDocument(), {
    'weights.safetensors': constantWeights(4),
  });
  await assert.rejects(
    () => ModelLoader.load('weights.safetensors', {
      graphUrl: 'graph.json', fetch: fetchDoc, dimensionBounds: { B: { max: 0 } },
    }),
    /must be a positive safe integer/,
  );
  // An empty interval is caught for the same reason: the authoritative parser
  // sees the overridden document, not the authored one.
  await assert.rejects(
    () => ModelLoader.load('weights.safetensors', {
      graphUrl: 'graph.json', fetch: fetchDoc, dimensionBounds: { B: { min: 9 } },
    }),
    /.+/,
  );
});
