...
import {
  ...
  STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS,
...
test('structural routes use generated canonical IDs and expose normalized variadic ports', () => {
  assert.equal(STRUCTURAL_SHAPE_FUNCTION_IDS.broadcastArithmetic, 'volvox.shape.broadcast-arithmetic.v1');
  assert.equal(STRUCTURAL_SHAPE_FUNCTION_IDS.broadcastComparison, 'volvox.shape.broadcast-comparison.v1');
  assert.equal(STRUCTURAL_SHAPE_FUNCTION_IDS.transpose, operatorShapeFunctionIds.Transpose);
  assert.equal(STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS.Sub, operatorShapeFunctionIds.Sub);
  assert.equal(STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS.ReduceSum, operatorShapeFunctionIds.ReduceSum);
  assert.equal(STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS.GatherElements, operatorShapeFunctionIds.GatherElements);
  for (const [operator, shapeFunctionId] of Object.entries(STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS)) {
  ...
  assert.equal(Object.isFrozen(STRUCTURAL_OPERATOR_SHAPE_FUNCTIONS), true);
});
