...
test('browser graph loading accepts canonical W8A8 QConv2D and rejects noncanonical sidecar metadata', () => {
  const graph = new RuntimeGraph();
  const tensors = new Map();
  ...
  assert.throws(
    ...
    /must use canonical I8\/U8 NHWC\/OHWI storage/,
  );
});
...
test('browser W8A8 gate fails closed on generic byte activations but preserves explicit boundaries and W8A32 weights', () => {
  ...
  assert.throws(
    ...
    /canonical I8\/U8 NHWC\/OHWI storage/,
  );
});
