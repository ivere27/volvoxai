...
function loadAndValidateCorpus() {
  ...
  assert.deepEqual(manifestOperators, new Set(Object.keys(DIRECT_OPERATOR_SHAPE_FUNCTIONS)));
  ...
  assert.deepEqual(successfulOperators, manifestOperators,
    'every direct operator must have a legal shared vector');
  assert.deepEqual(failedOperators, manifestOperators,
    'every direct operator must have an illegal shared vector');
  return vectors;
}

const vectors = loadAndValidateCorpus();

test('shared direct success vectors match generated IDs and immutable TS inference', () => {
...
test('shared direct failure vectors have exact TS codes and paths', () => {
