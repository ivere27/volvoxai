...
🔬 두 공식 모두 출시 프로바이더가 구현합니다. 역양자화를 의사코드로 쓰면:
...
끝에서만 `DequantizeLinear` 가 최종 `scores`/`boxes` 를 읽을 수 있는 실수로 되돌립니다. 이 typed
W8A8 계약은 네이티브 CPU와 WASM이 공유하는 이식 가능한 C 커널에 구현되어 있고, WebGPU와 네이티브 GPU
프로바이더도 검증된 packed-byte route를 제공합니다. 지원하지 않는 descriptor는 거부하며, 조용히 fp32로
넓히거나 다른 프로바이더로 보내지 않습니다.
...
> 그 4배는 **디스크 위 가중치** 이야기입니다; 실행 시 활성화 메모리는 그래프가 typed W8A8 섬을
> 유지하는지, 명시적 역양자화 경계를 지나는지(§6.4), 그리고 선택한 프로바이더가 어떤 구현을 검증했는지에
> 달렸습니다.
