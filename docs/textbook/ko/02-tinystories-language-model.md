...
예측하는 과정을 따라갑니다. 여기 나오는 모든 연산은 출시 프로바이더가 공유하는 같은 계약을 따릅니다.*
...
🔧 신경망은 글자를 읽지 못하고 숫자를 읽습니다. **토크나이저**는 텍스트를 **토큰**
(자주 쓰이는 단어 조각)으로 나누고, 어휘와 순위가 있는 **병합 규칙**을 이용해 정수 ID로 바꿉니다.
이것이 **Byte-Pair Encoding(BPE)** 입니다. 예제는 `examples/tinystories/tools/export_tokenizer.py`로
asset을 만듭니다. 재사용 가능한 토크나이저는 `native/src/runtime/tokenizer.c`에서 실행되며,
두 프로필 모두 `VxTextService`로 제공합니다. 모델에 맞는 asset과 프롬프트 정책은 호출자가 고릅니다.
...
> 더 적용할 게 없을 때까지 탐욕적으로 붙입니다. `VxTextService`는 생성된 `vocab.bin`과
> `merges.txt`를 받아 인코딩·디코딩하고, 추론 입력에는 그 토큰 ID와 위치를 넘깁니다.
> 모델에 맞는 asset으로 예상 ID를 확인하세요. 호스트들이 C 토크나이저를 공유한다는 사실만으로
> 모든 모델의 토큰화 규칙과 호환된다고 보장할 수는 없습니다.
...
순수한 표 조회입니다. 다음은 그 전체 계산을 보여주는 개념적 루프입니다:
...
가중치로 바꾼 뒤, 그 토큰들의 Value를 가중 혼합해 돌려줍니다. 인과(causal) 계산을 가볍게 주석 달면:
...
`GELU`가 비선형입니다 — 작은 음수는 새어 나가게 하고 양수는 통과시키는 매끄러운
...
> 통째로 지웁니다 — 텐서가 어디서나 *진짜로* 여섯 행입니다. 과거 padded-static 기준 측정은 이때
> 사라지는 상한을 기록했습니다: `S = 64` 를 512로 패딩하면 폭 128 시퀀스 Linear가 패딩본에서 **7.9배 느립니다**
...
🔧 이제 다음 토큰에 대한 50257개 점수가 있습니다. 애플리케이션 코드는 가장 단순한
규칙인 **탐욕/argmax**를 쓸 수 있습니다 — 그냥 가장 높은 것을 택합니다:

```c
int best_id = 0; float best_val = -1e30f;
for (int i = 0; i < vocab_count; i++)
    if (logits[i] > best_val) { best_val = logits[i]; best_id = i; }   // argmax
// best_id가 다음 토큰; 애플리케이션 어휘로 디코드:
printf("%s", decode_token(app_tokenizer, best_id)); // 애플리케이션 helper
```
...
> 것만 계산하게 합니다. VolvoxAI에서는 각 디코드 스트림이 생성된 컨텍스트 ID를 가집니다.
> 생성된 `DecodePrefill` 연산은 프롬프트를 처리하고 `DecodeStep` 은 한 위치를 진행하며,
> `ResetDecode` 는 그 상태를 지웁니다. 성공한 호출은 결과 ID를 반환하고, 이름 붙은 스냅숏은
> `ReadOutput` 으로 읽습니다. 수학은 동일하고, 캐시는 반복
...
- 모든 프로바이더는 같은 작은 연산을 구현합니다 — `MatMul`, `SDPA`, `LayerNorm`, `GELU`, `Add`,
