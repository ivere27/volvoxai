...
*위치* 를 섞은 64개 숫자 벡터가 들어 있습니다. 이 텐서 `hidden_0 [1,256,64]` 가 **잔차 스트림
...
> 것만 계산하게 합니다. VolvoxAI의 네이티브 런타임은 이 분리를 일반 시퀀스 API로 노출합니다:
> `volvoxai_engine_forward_prefix()` 는 프롬프트 행들을 한 번 처리하고,
> `volvoxai_engine_forward_row()` 는 한 행씩 나아가며, `volvoxai_engine_tensor_row_f32()` 는
> 그래프의 선언된 출력에서 그 행을 읽습니다. 수학은 동일하고, 캐시는 반복 작업만 피할 뿐입니다.
