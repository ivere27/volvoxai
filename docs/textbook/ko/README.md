...
- 🌱 **"AI가 무엇인지 그냥 이해하고 싶다."** 모든 장에서 🌱 **아이디어** 절만 쭉 읽으세요. 코드도
  ...
  이해하게 됩니다.
- 🔧 **"코드를 조금 다룰 줄 알고, 실제로 돌아가는 걸 보고 싶다."** 🌱 **+** 🔧 를 읽으세요. 연산 계약에서
  WASM·WebGPU·네이티브 구현까지 따라갈 수 있게 됩니다.
...
1. **[기초](01-foundations.md)** 🌱🔧🔬 — 순전파란 무엇인가. 텐서(tensor), 그래프, 연산(operation).
   VolvoxAI의 멘탈 모델과 명시적 프로바이더. 모델이 `graph.json`과 가중치로 저장되는 방식, 그리고
...
8. **[브라우저 엔진 내부](08-inside-the-engine.md)** 🌱🔧🔬 — WASM과 WebGPU 프로바이더, *소박한(naive)*
