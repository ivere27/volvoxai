...
>
> *작은 예.* 정답 다음 단어가 "was" 이고 모델이 확률 0.5를 준다고 합시다. 교차 엔트로피 손실은
> `−ln(0.5) ≈ 0.69`. 더 확신해 0.9였다면 손실은 `−ln(0.9) ≈ 0.11`(더 좋음). 완벽히 확신하며 맞으면 →
> 손실 0; 확신하며 *틀리면* → 손실 거대. 손실은 결국 "모델이 정답에 얼마나 놀랐나" 입니다.
...
벌합니다. 🔬 Trainer는 이것을 손실 *과* 첫 기울기를 함께 돌려주는 융합 커널로 계산합니다:
...
> 🔬 **뜯어보기: 하나로 융합된, 안정적인 커널.** 교차 엔트로피는 `−log softmax(logits)[target]` 이고,
> 그 기울기는 저 깔끔한 `softmax(logits) − onehot(target)` 으로 접힙니다 — 그래서 커널은 확률을 **한
> 번** 계산해 손실과 씨앗 기울기 *둘 다* 에 재사용합니다, 한 번의 순회로. 소프트맥스는 **log-sum-exp**
> 형태(§2.5b처럼 행 최댓값 빼기)로 돌려 큰 로짓이 `exp` 를 넘치지 않게 하고, `correct` 개수(argmax가
> 정답을 맞혔나?)도 세어 손실 옆에 정확도를 공짜로 줍니다.
...
> 🔬 **뜯어보기: 왜 *역방향* 이고, 왜 값싼가.** 슬로프를 앞→뒤로(순방향 모드) 계산할 수도 있지만,
> 그건 *입력 가중치마다* 전체 순회 한 번이 들어 — 수백만 개면 가망이 없습니다. 하나의 손실에서
> **거꾸로** 가면 **모든** 가중치의 기울기를 **한 번**에 냅니다. 각 연산의 `dLoss/d(출력)` 을 그것에
> 공급된 모든 가중치에 재사용하니까요. 그 비대칭 — 스칼라 하나 나가고, 손잡이 수백만 개 들어옴 — 이
> 학습이 감당 가능한 이유 전부이고, 역전파가 순전파의 백만 배가 아니라 대략 같은 비용인 이유입니다.
...
> 🔬 **뜯어보기: 형태가 맞아떨어진다.** `x` 형태 `[m, k]`, `W` 형태 `[k, n]` 이면 `y = x·W` 는
> `[m, n]`. 그러면 `dx = dy · Wᵀ` 는 `[m, n]·[n, k] = [m, k]`(`x` 와 일치)이고, `dW = xᵀ · dy` 는
> `[k, m]·[m, n] = [k, n]`(`W` 와 일치)입니다. 기울기는 언제나 자신이 기울기인 대상과 **같은 형태**
> 입니다 — 어떤 쌍을 읽든 공짜 검산이 됩니다.
...
> 🔬 **뜯어보기: scatter-add는 병렬에서 조심해야 한다.** 두 위치가 *같은* 토큰을 담을 수 있어, 두
> 스레드가 같은 표 행에 동시에 더할 수 있습니다 — 데이터 경쟁. GPU 임베딩-역방향 커널은 그 쓰기를
> 직렬화해(atomic이나 행별 리덕션) 어떤 갱신도 잃지 않게 하고, CPU 커널은 순서대로 돕니다. "`+=` 가
> 어떤 순서로 일어나나" 를 일부러 다뤄야 하는 유일한 역방향 쌍입니다.
...
🔬 `SDPA'` 가 가장 바쁜 쌍입니다 — 기울기를 소프트맥스 *와*
Q·K 유사도 *와* value 혼합을 통해 되돌려야 하니까요 — 하지만 개념적으로는 여전히 "`dLoss/d(어텐션
출력)` 이 주어지면 `dLoss/d(qkv)` 를 만든다" 일 뿐입니다. 순방향 정보 흐름을 막던 인과 마스크(§2.5b)
가 역방향에서도 똑같이 막습니다.

> 🔬 **뜯어보기: 역방향은 순방향의 남은 것이 필요하다.** 소프트맥스를 미분하려면 `SDPA'` 는 순방향의
> 어텐션 **확률** 이 필요합니다. 그것을 **저장** 하거나(빠르지만 헤드당 `O(seq²)` 메모리) **재계산**
> 할 수 있습니다(메모리는 적고 계산은 더 — 플래시 어텐션의 발상). 일반 규칙: *모든* 쌍은 어떤 순방향
> 값(`x`, `y`, 또는 둘 다)이 필요하며, 그래서 학습이 추론보다 훨씬 많은 메모리를 씁니다 — 쌍이 실행될
> 때까지 활성화를 살려 둬야 하니까요.
...
- **정수 캐스트와 양자화** — 정수가 관여하는 캐스트는 기울기를 일부러 **멈춥니다**. 양자화 인지
  학습은 명시적인 가짜 양자화 스트레이트-스루 규칙을 사용합니다(7장).
- **비유한 값 가드** — Trainer는 (너무 큰 학습률이나 수치 폭주로 인한)
  NaN/Inf 기울기를, 가중치를 오염시키기 *전에* 트레이너가 탐지하게 합니다.

> 🔬 **뜯어보기: 메모리 청구서, 그리고 덜 내는 법.** 각 쌍이 순방향 활성화를 필요로 하므로(§4.5),
> 소박한 역전파는 *모든* 중간 텐서를 살려 둡니다 — 대개 학습의 지배적 메모리 비용. **그래디언트
> 체크포인팅** 은 계산과 메모리를 맞바꿉니다: 활성화 몇 개만 저장하고 나머지는 역방향 중에 *재계산*
> 합니다. 전부 저장하면 안 들어갈 모델을 학습시키는 표준 지렛대입니다.
