...
flowchart TD
    ...
    I --> T1["Tier 1 · WebNN<br/>그래프를 브라우저 ML API에 → NPU/GPU/CPU"]
    I --> T2["Tier 2 · WebGPU<br/>노드당 컴퓨트 파이프라인 하나, 전부 GPU에서"]
    I --> T3["Tier 3 · WASM SIMD<br/>선형 메모리 위 컴파일된 C 커널"]
    I --> T4["Tier 4 · 순수 JS<br/>참조 커널 — 느리지만 항상 올바름"]
    T1 -.폴백.-> T3
    T3 -.폴백.-> T4
```
...
- **Tier 3 — WASM** (`ts/backends/WasmEngine.ts` + `native/src/kernels/*.c`): 같은 연산을 SIMD로
  WebAssembly에 컴파일. 범프 할당기가 모든 텐서를 하나의 평평한 선형 메모리 블록에 넣고, `execute()`
  가 노드당 컴파일된 C 커널을 호출. 순수 JS보다 종종 5–50배 빠름.
- **Tier 2 — WebGPU** (`ts/backends/GraphExecutor.ts` + `shaders/inference/*.wgsl`): 각 연산이 GPU
  **컴퓨트 셰이더** 가 됨. 컴파일 시점에 모든 가중치를 VRAM에 올리고 *노드당 파이프라인 하나* 를
  빌드, `execute()` 가 하나의 커맨드 스트림으로 재생, **노드 사이 CPU 왕복 없이** — 모델 전체가
  GPU에 상주. 필요한 셰이더가 없으면 현재 실행기는 경고하고 그 노드를 건너뜀. 지원 안 되는 연산이
  필요한 모델엔 WASM/CPU 사용.
- **Tier 1 — WebNN** (`ts/backends/WebNNEngine.ts`): 그래프를 브라우저 자체 신경망 API에 넘김. 전용
  **NPU** 로 보낼 수 있음. 지원 안 되는 연산은 하위 계층으로 떨어짐.

🔬 설계 원칙: **미리 유효한 백엔드를 고른다.** WebNN은 그래프 컴파일 실패 시 떨어질 수 있고, WASM은
순수 JS 헬퍼로 폴백할 수 있습니다. WebGPU는 셰이더가 커버하는 연산의 그래프에 써야 합니다.
