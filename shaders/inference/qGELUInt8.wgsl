// Canonical byte-domain GELU. QGELU permits only the portable erf semantics:
// the Abramowitz-Stegun 7.1.26 polynomial below has the same F32 constants and
// operation order as qGELU.js and the qgelu_i8u8 WASM ABI. Activations are
// packed as four I8/U8 values per u32; each invocation owns one output word.
...
fn erf_approx_f32(value : f32) -> f32 {
  let sign = select(-1.0, 1.0, value >= 0.0);
  let x = abs(value);
  let t_denominator = 1.0 + 0.3275911 * x;
  let t = 1.0 / t_denominator;
  var polynomial = 1.061405429 * t;
  polynomial = polynomial - 1.453152027;
  polynomial = polynomial * t;
  polynomial = polynomial + 1.421413741;
  polynomial = polynomial * t;
  polynomial = polynomial - 0.284496736;
  polynomial = polynomial * t;
  polynomial = polynomial + 0.254829592;
  polynomial = polynomial * t;
  let squared = x * x;
  let tail = exp(-squared);
  return sign * (1.0 - polynomial * tail);
}
...
fn qgelu_one(index : u32) -> u32 {
  ...
  let erf_value = erf_approx_f32(erf_input);
  let cdf = 0.5 * (1.0 + erf_value);
  let gelu = x * cdf;
  let scaled = gelu / params.output_scale;
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  var packed = qgelu_one(first_element);
  packed = packed | (qgelu_one(first_element + 1u) << 8u);
  packed = packed | (qgelu_one(first_element + 2u) << 16u);
  packed = packed | (qgelu_one(first_element + 3u) << 24u);
