...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
    ...
    let q_base = b_idx * params.seq_len_q * params.d_model;
    let kv_base = b_idx * params.seq_len_kv * params.d_model;

    var q_src : array<f32, 64>;
    for (var i = 0u; i < params.d_model; i = i + 1u) {
        q_src[i] = q_in[q_base + q_idx * params.d_model + i];
    }
    ...
    for (var k_idx = 0u; k_idx < params.seq_len_kv; k_idx = k_idx + 1u) {
        ...
        for (var i = 0u; i < params.d_model; i = i + 1u) {
            kv_src[i] = kv_in[kv_base + k_idx * params.d_model + i];
        }
        var score = 0.0;
        for (var d = 0u; d < params.head_dim; d = d + 1u) {
    ...
    for (var d = 0u; d < params.head_dim; d = d + 1u) {
        ...
        output[q_base + q_idx * params.d_model + h_idx * params.head_dim + d] = out_val;
    }
}
