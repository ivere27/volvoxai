...
// Browser-only typed Where/Mask. The native-facing where.wgsl has an F32
// condition contract; this variant reads raw 32-bit condition words so F32 and
// I32 condition tensors retain their declared semantics.
...
struct Params {
  // elements, canonical condition_dtype (18=F32, 16=I32), pad, pad
  dimensions : vec4<u32>,
}
...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
  ...
  output_words[index] = select(b_words[index], a_words[index],
    condition_is_nonzero(index));
}
