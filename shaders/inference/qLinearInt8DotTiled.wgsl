...
fn signed_word(word:u32,dtype:u32)->u32 {
  if(dtype == 5u){return word^0x80808080u;} return word;
}
fn centered_zero_point(zero_point:i32,dtype:u32)->i32 {
  if(dtype == 5u){return zero_point-128;} return zero_point;
}
...
fn requantize_scaled_bits(bits:u32,zero_point:i32,minimum:i32,maximum:i32)->i32 {
  let magnitude=bits&0x7fffffffu;
  let negative=(bits&0x80000000u)!=0u;
  if(magnitude>0x7f800000u){return clamp(zero_point,minimum,maximum);}
  if(magnitude==0x7f800000u){return select(maximum,minimum,negative);}
  let exponent_bits=magnitude>>23u;
  if(exponent_bits==0u){return clamp(zero_point,minimum,maximum);}
  let exponent=i32(exponent_bits)-127;
  if(exponent < -2){return clamp(zero_point,minimum,maximum);}
  if(exponent>=9){return select(maximum,minimum,negative);}
  ...
  if(zero_point==0){
    ...
    let zero_is_larger=zero_high!=0u||(zero_high==0u&&zero_low>=significand);
    if(zero_is_larger){
      sum_low=zero_low-significand;
  ...
  if(sum_low==0u&&sum_high==0u){return 0;}
  let top_bit=select(
    highest_bit_u32(sum_low),32u+highest_bit_u32(sum_high),sum_high!=0u);
  if(top_bit>23u){
    let discard_count=top_bit-23u;
    var rounded_significand=
      (sum_low>>discard_count)|(sum_high<<(32u-discard_count));
    let discarded=sum_low&((1u<<discard_count)-1u);
    let addition_halfway=1u<<(discard_count-1u);
    if(discarded>addition_halfway||
       (discarded==addition_halfway&&(rounded_significand&1u)!=0u)){
  ...
  let integer_magnitude=(sum_low>>shift)|(sum_high<<(32u-shift));
  let remainder=sum_low&((1u<<shift)-1u);
  let halfway=1u<<(shift-1u);
  var rounded=select(i32(integer_magnitude),-i32(integer_magnitude),sum_negative);
