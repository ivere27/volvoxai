// Canonical byte-domain SiLU. Activations are packed as four I8/U8 values per
// u32; each invocation owns one destination word, so byte lanes never race.
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  var packed = qsilu_one(first_element);
  packed = packed | (qsilu_one(first_element + 1u) << 8u);
  packed = packed | (qsilu_one(first_element + 2u) << 16u);
  packed = packed | (qsilu_one(first_element + 3u) << 24u);
