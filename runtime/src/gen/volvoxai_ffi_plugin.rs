...
pub trait VolvoxAIServicePlugin: Send + Sync + 'static {
    ...
    fn recognize_c_t_c(&self, request: CTCRequest) -> Result<CTCResponse, FfiError>;
}

static PLUGIN_VOLVOX_A_I_SERVICE: std::sync::OnceLock<Box<dyn VolvoxAIServicePlugin>> = std::sync::OnceLock::new();

pub fn register_volvox_a_i_service_plugin(plugin: impl VolvoxAIServicePlugin) {
    let _ = PLUGIN_VOLVOX_A_I_SERVICE.set(Box::new(plugin));
}

fn get_volvox_a_i_service_plugin() -> Option<&'static dyn VolvoxAIServicePlugin> {
    PLUGIN_VOLVOX_A_I_SERVICE.get().map(|p| p.as_ref())
}
...
pub extern "C" fn Synurang_Invoke_VolvoxAIService(
    ...
    let plugin = match get_volvox_a_i_service_plugin() {
        Some(p) => p,
    ...
    match method_str {
        ...
        "/volvoxai.v1.VolvoxAIService/RecognizeCTC" => {
            match CTCRequest::decode(input.as_slice()) {
                Ok(req) => match plugin.recognize_c_t_c(req) {
                    Ok(resp) => {
                        let mut buf = Vec::new();
                        if resp.encode(&mut buf).is_ok() {
                            success_response(resp_len, &buf)
