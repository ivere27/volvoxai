...
void tanh_f32(const float* input, float* output, int n) {
    for (int i = 0; i < n; i++) output[i] = fast_tanhf(input[i]);
}
...
void rmsnorm_f32(const float* input, const float* weight, float* output, int seq_len, int d_model, float eps) {
    for (int s = 0; s < seq_len; s++) {
        float var = 0.0f;
        for (int d = 0; d < d_model; d++) {
            var += input[s*d_model + d] * input[s*d_model + d];
        }
        var /= (float)d_model;
        float inv_std = 1.0f / fast_sqrtf(var + eps);
        for (int d = 0; d < d_model; d++) {
            output[s*d_model + d] = input[s*d_model + d] * inv_std * weight[d];
        }
    }
}
