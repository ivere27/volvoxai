...
typedef void (*vx_parallel_fn)(void *ctx, int begin, int end);
...
static VxThreadPool vx_pool = {
  .mu = PTHREAD_MUTEX_INITIALIZER,
  .cv_start = PTHREAD_COND_INITIALIZER,
  .cv_done = PTHREAD_COND_INITIALIZER
};

static int vx_num_threads_override = 0;   // set via --num_threads; 0 = auto-detect
static int vx_thread_count_cached = 0;

// Set the worker-thread count (from the CLI --num_threads flag). Call before the first
// parallel_for / pool init. Resets the cache so the new value takes effect.
void vx_set_num_threads(int n) {
  vx_num_threads_override = n > 0 ? n : 0;
  vx_thread_count_cached = 0;
}

static int vx_native_thread_count(void) {
  if (vx_thread_count_cached > 0) return vx_thread_count_cached;
  int n = vx_num_threads_override;
  if (n <= 0) {
  ...
  if (n < 1) n = 1;
  if (n > VX_MAX_THREADS) n = VX_MAX_THREADS;
  vx_thread_count_cached = n;
  return vx_thread_count_cached;
}

static int vx_pool_take_chunk(int *begin, int *end) {
  pthread_mutex_lock(&vx_pool.mu);
  int b = vx_pool.next;
  if (b >= vx_pool.total) {
    pthread_mutex_unlock(&vx_pool.mu);
    return 0;
  }
  int e = b + vx_pool.grain;
  if (e > vx_pool.total) e = vx_pool.total;
  vx_pool.next = e;
  pthread_mutex_unlock(&vx_pool.mu);
  *begin = b;
  *end = e;
...
static void vx_pool_run_chunks(void) {
  int b, e;
  while (vx_pool_take_chunk(&b, &e)) vx_pool.fn(vx_pool.ctx, b, e);
}

static void *vx_worker_main(void *arg) {
  (void)arg;
  int seen_generation = 0;
  for (;;) {
    pthread_mutex_lock(&vx_pool.mu);
    while (!vx_pool.stop && vx_pool.generation == seen_generation)
      pthread_cond_wait(&vx_pool.cv_start, &vx_pool.mu);
    if (vx_pool.stop) {
      pthread_mutex_unlock(&vx_pool.mu);
      return NULL;
    }
    seen_generation = vx_pool.generation;
    pthread_mutex_unlock(&vx_pool.mu);
    ...
    pthread_mutex_lock(&vx_pool.mu);
    vx_pool.remaining--;
    if (vx_pool.remaining == 0) pthread_cond_signal(&vx_pool.cv_done);
    pthread_mutex_unlock(&vx_pool.mu);
  }
}

static void vx_pool_init(void) {
  if (vx_pool.initialized) return;
  vx_pool.nthreads = vx_native_thread_count();
  vx_pool.nworkers = vx_pool.nthreads - 1;
  int created = 0;
  for (int i = 0; i < vx_pool.nworkers; i++) {
    if (pthread_create(&vx_pool.threads[i], NULL, vx_worker_main, NULL) != 0) break;
    created++;
  }
  vx_pool.nworkers = created;
  vx_pool.nthreads = created + 1;
  vx_pool.initialized = 1;
}

static void vx_parallel_for(int total, int grain, vx_parallel_fn fn, void *ctx) {
  int nthreads = vx_native_thread_count();
  if (total <= 0) return;
  if (grain < 1) grain = 1;
  if (nthreads <= 1 || total <= grain) {
    fn(ctx, 0, total);
    return;
  }
  vx_pool_init();
  ...
  vx_pool.grain = grain;
  vx_pool.next = 0;
  vx_pool.remaining = vx_pool.nthreads;
  vx_pool.generation++;
  pthread_cond_broadcast(&vx_pool.cv_start);
  pthread_mutex_unlock(&vx_pool.mu);
  ...
  vx_pool.remaining--;
  if (vx_pool.remaining == 0) pthread_cond_signal(&vx_pool.cv_done);
  while (vx_pool.remaining != 0) pthread_cond_wait(&vx_pool.cv_done, &vx_pool.mu);
  pthread_mutex_unlock(&vx_pool.mu);
}
#else
static void vx_parallel_for(int total, int grain, vx_parallel_fn fn, void *ctx) {
  (void)grain;
  if (total > 0) fn(ctx, 0, total);
}
#endif

// Public entry so other translation units (e.g. quant_cpu_opt.c) can use the same
// shared worker pool instead of running single-threaded.
void vx_kernels_parallel_for(int total, int grain, void (*fn)(void *, int, int), void *ctx) {
  vx_parallel_for(total, grain, fn, ctx);
}
...
static void conv2d_pointwise_f32(const float *in, float *out,
  ...
  if (work < 2000000L) {
    ...
    vx_parallel_for(out_c, 8, conv2d_pointwise_task, &ctx);
  }
}
...
static void conv2d_depthwise_f32(const float *in, float *out,
  ...
  if (work < 500000L) {
    ...
    vx_parallel_for(c, 8, conv2d_depthwise_task, &ctx);
  }
}
...
static void conv2d_pointwise_f16w(const float *in, float *out,
  ...
  else vx_parallel_for(out_c, 8, conv2d_pointwise_f16w_task, &ctx);
}
