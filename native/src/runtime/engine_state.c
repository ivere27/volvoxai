/* The engine instance itself: its mutexes and generation counters, adapter
 * effects, CPU typed workspaces, weight caches, options and shutdown.
 *
 * Distinct from runtime_state.c, which only tracks *which* engine is current
 * on this thread, and from engine_runtime.c, which is the translation unit
    ...
    if (!kernel_platform->configuration_valid) {
        ...
        vx_engine_log(
                ...
                requested && requested[0]
                    ? requested
                    : vx_kernel_isa_name(kernel_platform->requested_isa));
        return -1;
    }
...
int volvoxai_engine_init_with_weight_files(const char* graph_path,
    ...
    int rc = volvoxai_engine_init_with_weight_files_impl(
        graph_path, weight_file_paths, NULL, weight_file_count);
...
int volvoxai_engine_init_with_borrowed_weight_files(
        ...
        const SafetensorsFile* weight_files,
        const char* const* weight_file_paths,
        int weight_file_count) {
    if (!graph_path || !graph_path[0] || !weight_files ||
        weight_file_count < 0 || weight_file_count > MAX_WEIGHT_FILES ||
        (weight_file_count > 0 && !weight_file_paths)) return -1;
    for (int index = 0; index < weight_file_count; index++) {
        if (!weight_file_paths[index] || !weight_file_paths[index][0] ||
            weight_files[index].borrows_storage ||
            (weight_files[index].flags & SAFETENSORS_OPEN_READ_WRITE))
            return -1;
    }
    if (g_engine_route_lease) return -1;
    volvoxai_engine_model_lock();
    ...
    volvoxai_engine_metadata_lock();
    int rc = volvoxai_engine_init_with_weight_files_impl(
        graph_path, weight_file_paths, weight_files, weight_file_count);
    volvoxai_engine_metadata_unlock();
    volvoxai_engine_model_unlock();
    return rc;
}
...
static int volvoxai_engine_set_tensor_raw_impl(const char* name, int dtype, const void* data, size_t nbytes) {
    if (g_weight_files_borrowed || !name || (!data && nbytes > 0)) return -1;
...
int volvoxai_engine_add_model_tensor_raw_locked(const char* name, const int* shape,
    ...
    if (g_weight_files_borrowed || !name || !name[0] ||
        strlen(name) >= sizeof(g_t[0].name) ||
...
int volvoxai_engine_remove_model_tensor_locked(const char* name) {
    if (g_weight_files_borrowed || !name || !name[0]) return -1;
    int rc = -1;
    int mutation_prepared = 0;
...
static int volvoxai_engine_adapter_merge_impl(const char* version_id) {
    if (g_weight_files_borrowed || !version_id || !version_id[0] ||
        g_merged_adapter_count) return -1;
...
static int volvoxai_engine_adapter_unmerge_impl(void) {
    if (g_weight_files_borrowed || !g_merged_adapter_count) return -1;
    char restore[128];
