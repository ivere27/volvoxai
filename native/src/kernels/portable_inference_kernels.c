 ...
 * Division deliberately uses IEEE division without the legacy epsilon from
 * div_f32(), matching the JavaScript portable reference. */
...
static double vx_pf_typed_value(const void *data, uint32_t dtype, size_t index) {
    if (dtype == 0u) return vx_pf_load_f32_le(data, index);
    if (dtype == 1u) return (int32_t)vx_pf_load_u32_le(data, index);
    if (dtype == 2u) return ((const int8_t *)data)[index];
...
static int vx_pf_w8a8_byte_dtype(uint32_t dtype) {
    return dtype == 2u || dtype == 3u;
}

static int vx_pf_w8a8_zero_point_valid(int32_t value, uint32_t dtype) {
    if (dtype == 2u) return value >= -128 && value <= 127;
    if (dtype == 3u) return value >= 0 && value <= 255;
    return 0;
}

static int32_t vx_pf_w8a8_byte_value(const void *data, uint32_t dtype,
        size_t index) {
    return dtype == 2u ? (int32_t)((const int8_t *)data)[index] :
...
static int32_t vx_pf_w8a8_round_ties_even(float value) {
...
static int32_t vx_pf_w8a8_quantize_transformed(float transformed,
    ...
    return vx_pf_w8a8_round_ties_even(transformed);
}

static void vx_pf_w8a8_store_byte(void *output, uint32_t dtype,
        size_t index, int32_t value) {
    if (dtype == 2u) ((int8_t *)output)[index] = (int8_t)value;
...
int qlinear_i8u8(const void *input, const void *weight, const int32_t *bias,
    ...
    if (!input || !weight || !bias || !weight_scales || !weight_zero_points || !output ||
        !rows || !d_in || !d_out || !vx_pf_w8a8_byte_dtype(input_dtype) ||
        !vx_pf_w8a8_byte_dtype(weight_dtype) || !vx_pf_w8a8_byte_dtype(output_dtype) ||
        !vx_pf_finite_f32(input_scale) || input_scale <= 0.0f ||
        !vx_pf_finite_f32(output_scale) || output_scale <= 0.0f ||
        !vx_pf_w8a8_zero_point_valid(input_zero_point, input_dtype) ||
        !vx_pf_w8a8_zero_point_valid(output_zero_point, output_dtype) ||
        !vx_pf_mul_size(&input_elements, d_in) ||
        !vx_pf_mul_size(&weight_elements, d_in) ||
        !vx_pf_mul_size(&output_elements, d_out)) return 0;
    output_minimum = output_dtype == 2u ? -128 : 0;
    output_maximum = output_dtype == 2u ? 127 : 255;
    for (uint32_t column = 0; column < d_out; column++) {
        if (!vx_pf_finite_f32(weight_scales[column]) || weight_scales[column] <= 0.0f ||
            !vx_pf_w8a8_zero_point_valid(weight_zero_points[column], weight_dtype)) return 0;
    }
    for (uint32_t row = 0; row < rows; row++) {
        ...
        for (uint32_t column = 0; column < d_out; column++) {
            ...
            for (uint32_t dimension = 0; dimension < d_in; dimension++) {
                int32_t input_value = vx_pf_w8a8_byte_value(input, input_dtype,
                    input_offset + dimension);
                int32_t weight_value = vx_pf_w8a8_byte_value(weight, weight_dtype,
            ...
            }
            {
                ...
                int32_t quantized;
                quantized = vx_pf_w8a8_quantize_transformed(transformed,
                    output_minimum, output_maximum, output_zero_point);
                vx_pf_w8a8_store_byte(output, output_dtype, output_offset + column, quantized);
            }
        }
    }
    return 1;
...
int qembedding_i8u8(const int32_t *tokens_data, const void *weight,
    ...
    if (!tokens_data || !weight || !weight_scales || !weight_zero_points || !output ||
        !tokens || !vocab || !hidden || !vx_pf_w8a8_byte_dtype(weight_dtype) ||
        !vx_pf_w8a8_byte_dtype(output_dtype) ||
        !vx_pf_finite_f32(output_scale) || output_scale <= 0.0f ||
        !vx_pf_w8a8_zero_point_valid(output_zero_point, output_dtype) ||
    ...
    for (uint32_t row = 0; row < vocab; row++) {
        if (!vx_pf_finite_f32(weight_scales[row]) || weight_scales[row] <= 0.0f ||
            !vx_pf_w8a8_zero_point_valid(weight_zero_points[row], weight_dtype)) return 0;
    }
    ...
    for (uint32_t token = 0; token < tokens; token++) {
        ...
        for (uint32_t column = 0; column < hidden; column++) {
            const int32_t centered = vx_pf_w8a8_byte_value(weight, weight_dtype,
            ...
            const int32_t quantized = vx_pf_w8a8_quantize_transformed(transformed,
                output_minimum, output_maximum, output_zero_point);
            vx_pf_w8a8_store_byte(output, output_dtype, output_offset + column, quantized);
        }
    }
...
int qadd_i8u8(const void *a, const void *b, void *output, uint32_t elements,
    ...
    if (!a || !b || !output || !elements || !vx_pf_w8a8_byte_dtype(a_dtype) ||
        !vx_pf_w8a8_byte_dtype(b_dtype) || !vx_pf_w8a8_byte_dtype(output_dtype) ||
        relu > 2u ||
        !vx_pf_finite_f32(a_scale) || a_scale <= 0.0f ||
        !vx_pf_finite_f32(b_scale) || b_scale <= 0.0f ||
        !vx_pf_finite_f32(output_scale) || output_scale <= 0.0f ||
        !vx_pf_w8a8_zero_point_valid(a_zero_point, a_dtype) ||
        !vx_pf_w8a8_zero_point_valid(b_zero_point, b_dtype) ||
        !vx_pf_w8a8_zero_point_valid(output_zero_point, output_dtype)) return 0;
    output_minimum = output_dtype == 2u ? -128 : 0;
    output_maximum = output_dtype == 2u ? 127 : 255;
    if (relu >= 2u) {
        const float relu6_scaled = 6.0f / output_scale;
        const float relu6_transformed = relu6_scaled + (float)output_zero_point;
        relu6_upper = vx_pf_w8a8_quantize_transformed(relu6_transformed,
            output_minimum, output_maximum, 0);
    }
    for (uint32_t index = 0; index < elements; index++) {
        const float a_delta = (float)(vx_pf_w8a8_byte_value(a, a_dtype, index) -
            a_zero_point);
        const float b_delta = (float)(vx_pf_w8a8_byte_value(b, b_dtype, index) -
            b_zero_point);
        ...
        const float transformed = scaled + (float)output_zero_point;
        int32_t quantized = vx_pf_w8a8_quantize_transformed(transformed,
            output_minimum, output_maximum, output_zero_point);
        if (relu) {
            if (quantized < output_zero_point) quantized = output_zero_point;
            if (relu >= 2u && quantized > relu6_upper) quantized = relu6_upper;
        }
        vx_pf_w8a8_store_byte(output, output_dtype, index, quantized);
    }
    return 1;
}
...
static int32_t vx_pf_qact_raw_value(uint8_t raw_byte, uint32_t input_dtype) {
    if (input_dtype == 2u && raw_byte >= 128u) return (int32_t)raw_byte - 256;
...
int qsilu_i8u8(const void *input, void *output, uint32_t elements,
    ...
    if (!input || !output || input == output || !elements ||
        !vx_pf_w8a8_byte_dtype(input_dtype) ||
        !vx_pf_w8a8_byte_dtype(output_dtype) ||
        !vx_pf_finite_f32(input_scale) || input_scale <= 0.0f ||
        !vx_pf_finite_f32(output_scale) || output_scale <= 0.0f ||
        !vx_pf_w8a8_zero_point_valid(input_zero_point, input_dtype) ||
...
static float vx_pf_affine_f32_at(const float *values, size_t index) {
    const unsigned char *bytes = (const unsigned char *)values + index * sizeof(float);
...
static int vx_pf_ranges_overlap(const void *left, size_t left_bytes,
...
int qgroupnorm_i8u8(const void *input, const float *weight, const float *bias,
    ...
    if (vx_pf_ranges_overlap(output, elements, input, elements) ||
        vx_pf_ranges_overlap(output, elements, weight, affine_bytes) ||
        vx_pf_ranges_overlap(output, elements, bias, affine_bytes)) return 0;
    for (uint32_t channel = 0; channel < channels; channel++) {
        if (!vx_pf_finite_f32(vx_pf_affine_f32_at(weight, channel)) ||
            !vx_pf_finite_f32(vx_pf_affine_f32_at(bias, channel))) return 0;
    }
    output_minimum = output_dtype == 2u ? -128 : 0;
    output_maximum = output_dtype == 2u ? 127 : 255;
    {
            ...
            for (uint32_t group = 0; group < groups; group++) {
                ...
                for (size_t spatial = 0; spatial < area; spatial++) {
                    ...
                    for (uint32_t local = 0; local < channels_per_group; local++) {
                        const float raw = (float)(vx_pf_w8a8_byte_value(input, input_dtype,
                            ...
                            if (real_variance < 0.0f) real_variance = 0.0f;
                            {
                                ...
                                for (size_t spatial = 0; spatial < area; spatial++) {
                                    ...
                                    for (uint32_t local = 0; local < channels_per_group; local++) {
                                        ...
                                        const float raw = (float)(vx_pf_w8a8_byte_value(input,
                                        ...
                                        const float affine = normalized *
                                            vx_pf_affine_f32_at(weight, channel) +
                                            vx_pf_affine_f32_at(bias, channel);
                                        const float output_scaled = affine / output_scale;
                                        const float transformed = output_scaled +
                                            (float)output_zero_point;
                                        const int32_t quantized = vx_pf_w8a8_quantize_transformed(
                                            transformed, output_minimum, output_maximum,
                                            output_zero_point);
                                        vx_pf_w8a8_store_byte(output, output_dtype,
...
static int32_t vx_pf_qsdpa_wasm_centered_dot(const void *q, const void *k,
    ...
    if (dimension + 8u <= dimensions) {
        ...
        const v128_t q16 = wasm_i16x8_sub(q_dtype == 2u
            ? wasm_i16x8_extend_low_i8x16(q8)
            : wasm_u16x8_extend_low_u8x16(q8), q_zp);
        const v128_t k16 = wasm_i16x8_sub(k_dtype == 2u
            ? wasm_i16x8_extend_low_i8x16(k8)
            : wasm_u16x8_extend_low_u8x16(k8), k_zp);
    ...
    }
    {
        ...
        for (; dimension < dimensions; dimension++) {
            const int32_t q_value = vx_pf_w8a8_byte_value(
                q, q_dtype, q_base + dimension) - q_zero_point;
            const int32_t k_value = vx_pf_w8a8_byte_value(
...
static void vx_pf_qsdpa_wasm_accumulate(float *accumulator, const void *v,
    ...
    for (uint32_t dimension = 0; dimension < dimensions; dimension += 4u) {
        ...
        const v128_t values16 = v_dtype == 2u
...
int qsdpa_i8u8(const void *q, const void *k, const void *v,
    ...
    if (!q || !k || !v || !output || !batch || !seq_q || !seq_kv ||
        ...
        !vx_pf_w8a8_byte_dtype(q_dtype) ||
        !vx_pf_w8a8_byte_dtype(k_dtype) ||
        !vx_pf_w8a8_byte_dtype(v_dtype) ||
        !vx_pf_w8a8_byte_dtype(output_dtype) ||
        ...
        !vx_pf_w8a8_zero_point_valid(q_zero_point, q_dtype) ||
        !vx_pf_w8a8_zero_point_valid(k_zero_point, k_dtype) ||
        !vx_pf_w8a8_zero_point_valid(v_zero_point, v_dtype) ||
        !vx_pf_w8a8_zero_point_valid(output_zero_point, output_dtype) ||
    ...
    {
        const int64_t q_low = (int64_t)(q_dtype == 2u ? -128 : 0) - q_zero_point;
        const int64_t q_high = (int64_t)(q_dtype == 2u ? 127 : 255) - q_zero_point;
        const int64_t k_low = (int64_t)(k_dtype == 2u ? -128 : 0) - k_zero_point;
        const int64_t k_high = (int64_t)(k_dtype == 2u ? 127 : 255) - k_zero_point;
    ...
    if (vx_pf_ranges_overlap(output, q_elements, q, q_elements) ||
        vx_pf_ranges_overlap(output, q_elements, k, kv_elements) ||
        vx_pf_ranges_overlap(output, q_elements, v, kv_elements) ||
        (mask_bytes && vx_pf_ranges_overlap(output, q_elements, mask, mask_bytes))) return 0;
    output_minimum = output_dtype == 2u ? -128 : 0;
    output_maximum = output_dtype == 2u ? 127 : 255;
            ...
            for (uint32_t head = 0; head < heads; head++) {
                ...
                for (uint32_t key = 0; key < seq_kv; key++) {
                    ...
                    for (uint32_t dimension = 0; dimension < head_dim; dimension++) {
                        const int32_t q_value = vx_pf_w8a8_byte_value(q, q_dtype,
                            q_row_base + head_base + dimension) - q_zero_point;
                        const int32_t k_value = vx_pf_w8a8_byte_value(k, k_dtype,
...
#else
                        for (uint32_t dimension = 0; dimension < head_dim; dimension++) {
                            const int32_t v_value = vx_pf_w8a8_byte_value(v, v_dtype,
                ...
                for (uint32_t dimension = 0; dimension < head_dim; dimension++) {
                    ...
                    vx_pf_w8a8_store_byte(output, output_dtype, output_index,
                                           quantized);
                }
            }
        }
...
int qargmax_i8u8(const void *input, int32_t *output, uint32_t outer,
    ...
    if (vx_pf_ranges_overlap(output, output_bytes, input, input_elements)) return 0;
    for (uint32_t outer_index = 0; outer_index < outer; outer_index++) {
        for (uint32_t inner_index = 0; inner_index < inner; inner_index++) {
            size_t input_index = ((size_t)outer_index * axis_size) * inner +
                inner_index;
            int32_t best_value = vx_pf_w8a8_byte_value(input, input_dtype,
                                                        input_index);
            uint32_t best_index = 0;
            for (uint32_t axis_index = 1; axis_index < axis_size; axis_index++) {
                int32_t value = vx_pf_w8a8_byte_value(input, input_dtype,
...
int qmaskedmean_i8u8(const void *input, const int32_t *mask, void *output,
    ...
    if (vx_pf_ranges_overlap(output, output_elements, input, input_elements) ||
        vx_pf_ranges_overlap(output, output_elements, mask, mask_bytes)) return 0;
    output_minimum = output_dtype == 2u ? -128 : 0;
    output_maximum = output_dtype == 2u ? 127 : 255;
    for (uint32_t batch_index = 0; batch_index < batch; batch_index++) {
            ...
            if (keep_count) {
                ...
                for (uint32_t token = 0; token < sequence; token++) {
                    if (vx_pf_qmaskedmean_mask_i32_at(mask, mask_base + token) != 0) {
                        ...
                        centered_sum += vx_pf_w8a8_byte_value(input, input_dtype,
...
int requantize_linear_i8u8(const void *input, void *output, uint32_t elements,
    ...
    int32_t output_minimum, output_maximum;
    float multiplier;
    if (!input || !output || !elements || !vx_pf_w8a8_byte_dtype(input_dtype) ||
        !vx_pf_w8a8_byte_dtype(output_dtype) || !vx_pf_finite_f32(input_scale) ||
    ...
    output_maximum = output_dtype == 2u ? 127 : 255;
    for (uint32_t index = 0; index < elements; index++) {
        const float centered = (float)(vx_pf_w8a8_byte_value(input, input_dtype, index) -
            input_zero_point);
...
static int vx_pf_qconv_accumulator_i32_valid(const int32_t *bias,
    ...
    if (!weight_zero_points || !output_channels || !terms ||
        !vx_pf_w8a8_byte_dtype(input_dtype) ||
        !vx_pf_w8a8_byte_dtype(weight_dtype) ||
        !vx_pf_w8a8_zero_point_valid(input_zero_point, input_dtype)) return 0;
    input_low = (int64_t)(input_dtype == 2u ? -128 : 0) - input_zero_point;
    input_high = (int64_t)(input_dtype == 2u ? 127 : 255) - input_zero_point;
    ...
    for (uint32_t output_channel = 0; output_channel < output_channels;
        ...
        if (!vx_pf_w8a8_zero_point_valid(weight_zero_point, weight_dtype)) return 0;
        weight_low = (int64_t)(weight_dtype == 2u ? -128 : 0) - weight_zero_point;
        weight_high = (int64_t)(weight_dtype == 2u ? 127 : 255) - weight_zero_point;
...
int qconv2d_im2col_i8u8(const void *input, void *columns,
    ...
    if (!input || !columns || !batch || !input_height || !input_width ||
        ...
        vx_pf_ranges_overlap(input, input_elements, columns, column_elements)) return 0;
    ...
    padding_byte = input_dtype == 2u ? (uint8_t)(int8_t)input_zero_point :
...
int qconv2d_i8u8(const void *input, const void *weight, const int32_t *bias,
    ...
    if (!input || !weight || !weight_scales || !weight_zero_points || !output ||
        ...
        !vx_pf_w8a8_byte_dtype(input_dtype) || !vx_pf_w8a8_byte_dtype(weight_dtype) ||
        !vx_pf_w8a8_byte_dtype(output_dtype) || !vx_pf_finite_f32(input_scale) ||
            ...
            for (uint32_t output_x = 0; output_x < output_width; output_x++) {
                for (uint32_t output_channel = 0; output_channel < output_channels;
                        ...
                        for (uint32_t kernel_x = 0; kernel_x < kernel_width; kernel_x++) {
                            ...
                            for (uint32_t local_channel = 0; local_channel < input_per_group;
                                ...
                                size_t weight_index = weight_channel_offset +
                                    ...
                                    local_channel;
                                accumulator += (int64_t)(vx_pf_w8a8_byte_value(input, input_dtype,
                                    input_index) - input_zero_point) *
                                    (int64_t)(vx_pf_w8a8_byte_value(weight, weight_dtype,
                    ...
                    }
                    {
                        ...
                        vx_pf_w8a8_store_byte(output, output_dtype, output_index, quantized);
                    }
                }
            }
        }
...
static int vx_pf_quantized_byte_dtype(uint32_t dtype) {
    return dtype == 2u || dtype == 3u;
}
...
int copy_i8u8(const uint8_t *input, uint8_t *output, uint32_t elements,
        uint32_t dtype) {
    if (!input || !output || !elements || !vx_pf_quantized_byte_dtype(dtype)) return 0;
...
int concat_slice_i8u8(const uint8_t *input, uint8_t *output, uint32_t outer,
    ...
    size_t input_block;
    if (!input || !output || !vx_pf_quantized_byte_dtype(dtype) ||
        !vx_pf_concat_dimensions_valid(outer, input_axis, output_axis, inner,
            axis_offset, &input_block)) return 0;
    for (uint32_t group = 0; group < outer; group++) {
        size_t source = (size_t)group * input_block;
...
int resize_nearest2d_i8u8(const uint8_t *input, uint8_t *output, uint32_t batch,
    ...
    size_t input_elements = 1, output_elements = 1;
    if (!input || !output || !batch || !input_height || !input_width || !channels ||
        !output_height || !output_width || !vx_pf_quantized_byte_dtype(dtype)) return 0;
    if (!vx_pf_mul_size(&input_elements, batch) || !vx_pf_mul_size(&input_elements, input_height) ||
        !vx_pf_mul_size(&input_elements, input_width) || !vx_pf_mul_size(&input_elements, channels) ||
        !vx_pf_mul_size(&output_elements, batch) || !vx_pf_mul_size(&output_elements, output_height) ||
        !vx_pf_mul_size(&output_elements, output_width) || !vx_pf_mul_size(&output_elements, channels)) return 0;
...
int maxpool2d_i8u8(const uint8_t *input, uint8_t *output, uint32_t batch,
    ...
    if (!input || !output || !batch || !input_height || !input_width || !channels ||
        ...
        !vx_pf_quantized_byte_dtype(dtype)) return 0;
    if (!vx_pf_mul_size(&input_elements, batch) || !vx_pf_mul_size(&input_elements, input_height) ||
        !vx_pf_mul_size(&input_elements, input_width) || !vx_pf_mul_size(&input_elements, channels) ||
        !vx_pf_mul_size(&output_elements, batch) || !vx_pf_mul_size(&output_elements, output_height) ||
        !vx_pf_mul_size(&output_elements, output_width) || !vx_pf_mul_size(&output_elements, channels)) return 0;
