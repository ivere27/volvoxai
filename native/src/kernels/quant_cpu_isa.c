...
#include <arm_neon.h>
#define VX_QUANT_ARM_NEON 1
#else
#define VX_QUANT_ARM_NEON 0
#endif
...
#if VX_QUANT_ARM_NEON
static int vx_quant_arm_ranges_overlap(const void* left, size_t left_bytes,
...
static int32x4_t vx_quantize_transformed4_neon(
        float32x4_t value, float32x4_t scale, int32x4_t zero,
        float32x4_t minimum, float32x4_t maximum) {
    const uint32x4_t nan_lanes = vmvnq_u32(vceqq_f32(value, value));
    float32x4_t transformed = vaddq_f32(vdivq_f32(value, scale),
                                        vcvtq_f32_s32(zero));
    int32x4_t quantized;

    /* Keep the divide and add as the two canonical F32 operations.  Clamping
     * before conversion gives infinities the portable saturation behavior;
     * NaN lanes are selected away after conversion and map to zero point. */
    transformed = vmaxq_f32(transformed, minimum);
    transformed = vminq_f32(transformed, maximum);
    quantized = vcvtnq_s32_f32(transformed);
    return vbslq_s32(nan_lanes, zero, quantized);
}

static void vx_store_quantized16_neon(
        void* output, uint32_t output_dtype, uint32_t index,
        int32x4_t q0, int32x4_t q1, int32x4_t q2, int32x4_t q3) {
    if (output_dtype == VX_DTYPE_I8) {
        const int16x8_t q01 = vcombine_s16(vqmovn_s32(q0), vqmovn_s32(q1));
        const int16x8_t q23 = vcombine_s16(vqmovn_s32(q2), vqmovn_s32(q3));
        vst1q_s8((int8_t*)output + index,
                 vcombine_s8(vqmovn_s16(q01), vqmovn_s16(q23)));
    } else {
        const uint16x8_t q01 = vcombine_u16(vqmovun_s32(q0),
                                            vqmovun_s32(q1));
        const uint16x8_t q23 = vcombine_u16(vqmovun_s32(q2),
                                            vqmovun_s32(q3));
        vst1q_u8((uint8_t*)output + index,
                 vcombine_u8(vqmovn_u16(q01), vqmovn_u16(q23)));
    }
}

static uint32_t vx_quantize_linear_typed_neon(
        const float* input, float scale, int zero_point, void* output,
        uint32_t output_dtype, uint32_t elements, int minimum, int maximum) {
    const float32x4_t scales = vdupq_n_f32(scale);
    const int32x4_t zeros = vdupq_n_s32(zero_point);
    const float32x4_t minima = vdupq_n_f32((float)minimum);
    const float32x4_t maxima = vdupq_n_f32((float)maximum);
    uint32_t index = 0;

    for (; index + 16u <= elements; index += 16u) {
        const int32x4_t q0 = vx_quantize_transformed4_neon(
            vld1q_f32(input + index), scales, zeros, minima, maxima);
        const int32x4_t q1 = vx_quantize_transformed4_neon(
            vld1q_f32(input + index + 4u), scales, zeros, minima, maxima);
        const int32x4_t q2 = vx_quantize_transformed4_neon(
            vld1q_f32(input + index + 8u), scales, zeros, minima, maxima);
        const int32x4_t q3 = vx_quantize_transformed4_neon(
            vld1q_f32(input + index + 12u), scales, zeros, minima, maxima);
        vx_store_quantized16_neon(output, output_dtype, index,
                                  q0, q1, q2, q3);
    }
    return index;
}

static void vx_dequantize_store4_neon(
        int32x4_t values, int32x4_t zeros, float32x4_t scales,
        float* output) {
    vst1q_f32(output, vmulq_f32(
        vcvtq_f32_s32(vsubq_s32(values, zeros)), scales));
}

static uint32_t vx_dequantize_linear_typed_neon(
        const void* input, uint32_t input_dtype, float scale, int zero_point,
        float* output, uint32_t elements) {
    const float32x4_t scales = vdupq_n_f32(scale);
    const int32x4_t zeros = vdupq_n_s32(zero_point);
    ...
    if (input_dtype == VX_DTYPE_I8) {
        for (; index + 16u <= elements; index += 16u) {
            const int8x16_t bytes = vld1q_s8((const int8_t*)input + index);
            const int16x8_t low = vmovl_s8(vget_low_s8(bytes));
            const int16x8_t high = vmovl_s8(vget_high_s8(bytes));
            vx_dequantize_store4_neon(vmovl_s16(vget_low_s16(low)),
                                      zeros, scales, output + index);
            vx_dequantize_store4_neon(vmovl_s16(vget_high_s16(low)),
                                      zeros, scales, output + index + 4u);
            vx_dequantize_store4_neon(vmovl_s16(vget_low_s16(high)),
                                      zeros, scales, output + index + 8u);
            vx_dequantize_store4_neon(vmovl_s16(vget_high_s16(high)),
                                      zeros, scales, output + index + 12u);
        }
    } else {
        for (; index + 16u <= elements; index += 16u) {
            const uint8x16_t bytes = vld1q_u8((const uint8_t*)input + index);
            const uint16x8_t low = vmovl_u8(vget_low_u8(bytes));
            const uint16x8_t high = vmovl_u8(vget_high_u8(bytes));
            vx_dequantize_store4_neon(
...
uint32_t vx_dequantize_linear_typed_native_prefix(
        ...
        float* output, uint32_t elements) {
#if VX_QUANT_ARM_NEON
    if (input && output && elements >= 16u &&
        (input_dtype == VX_DTYPE_I8 || input_dtype == VX_DTYPE_U8) &&
        !vx_quant_arm_ranges_overlap(input, (size_t)elements,
                                     output, (size_t)elements * sizeof(float)) &&
        vx_kernel_platform()->has_neon) {
        return vx_dequantize_linear_typed_neon(input, input_dtype, scale,
                                               zero_point, output, elements);
    }
#elif VX_QUANT_X86_AVX2
...
uint32_t vx_quantize_linear_typed_native_prefix(
    ...
    if (input && output && elements >= 16u &&
        ...
        !vx_quant_arm_ranges_overlap(input,
                                     (size_t)elements * sizeof(float),
                                     output, (size_t)elements) &&
        vx_kernel_platform()->has_neon) {
        return vx_quantize_linear_typed_neon(
