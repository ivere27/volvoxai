...
function fakeEnvironment({
  ...
  const api = {
    VolvoxAI: {
      async createRuntime(options) {
        assert.deepEqual(options.backends, ['cpu-js']);
  ...
  class Session {
    ...
    async generate(options) {
      ...
      diagnostic({
        kind: 'compilation',
        report: compilation('cpu-js', 0, {
          ...(fallback ? { routeEvidence: route({ tierFallback: true }) } : {}),
          ...(candidateDeviceMismatch
            ? { candidateDevice: fixtureDevice('cpu-js', 'different-candidate') }
            : {}),
        }),
      });
      diagnostic({ kind: 'execution', report: execution('cpu-js', 'encoder-context', 0) });
      diagnostic({
        kind: 'compilation',
        report: compilation('cpu-js', 1, compilationDeviceMismatch
          ? { device: fixtureDevice('cpu-js', 'different-compilation') }
          : {}),
      });
        ...
        diagnostic({
          ...
          report: execution(
            ...
            executionDeviceMismatch && index === 0
              ? { device: fixtureDevice('cpu-js', 'different-execution') }
              : {},
          ),
        });
      }
...
test('reference integrity rejects corrupted router bytes and numeric summaries', async () => {
  ...
  const reference = createTinyReceiptSplitE2EReference(candidate, {
    provenance: {
      ...
      sourceFormat: 'tiny_receipt_vqa_split_kv_onnx_v2',
