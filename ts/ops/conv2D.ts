...
export function _cpuConv2D(node) {
    ...
    const [stride_y, stride_x] = normalizeSpatialPair(node.params.stride, 1);
    const [pad_y, pad_x] = normalizeSpatialPair(node.params.padding, 0);
    const pads = node.params.pads || [pad_y, pad_x, pad_y, pad_x];
    const [dil_y, dil_x] = normalizeSpatialPair(node.params.dilation, 1);
