...
export interface RuntimeGraphBuilderPackage {
  graph: GraphDocument;
 ...
 *
 * RuntimeGraphBuilder never owns a separate graph representation: every successful
...
export class RuntimeGraphBuilder {
  ...
  constructor(graph: RuntimeGraph = new RuntimeGraph()) {
    if (!(graph instanceof RuntimeGraph)) throw new Error("RuntimeGraphBuilder expects a RuntimeGraph instance.");
    this.graph = graph;
  }
  ...
  build({ validate = true }: { validate?: boolean } = {}): RuntimeGraph {
  ...
  toGraphPackage(): RuntimeGraphBuilderPackage {
