...
import type { AdapterVersion, RuntimeDType, RuntimeTypedArray } from '../types.js';
...
interface WebGPULanguageFeatures {
  has(feature: string): boolean;
}

interface RowStorageDescriptor {
  readonly offsetStride: number;
  readonly constantSize: number;
  readonly queryMaskPrefix: boolean;
}

type ExecutorTensor = Tensor & { _webgpuRow?: RowStorageDescriptor };

type SpatialParameter = number | number[];

interface ExecutorNodeParams {
  [name: string]: unknown;
...
type ExecutorGraph = Omit<Graph, 'nodes'> & { nodes: ExecutorNode[] };
type AdapterExecutionPlan = ReturnType<Graph['adapters']['_pinExecution']>;
...
export interface WebGPUExecutionOptions extends BackendExecutionOptions {
  adapter?: string | WebGPUAdapterSelector | null;
  adapters?: Array<string | WebGPUAdapterSelector | null>;
  training?: {
    pipelineOverrides?: Map<number, CompiledWebGPUPipeline>;
  };
}
...
interface IncrementalRowCandidate {
  nodeIndex: number;
  node: ExecutorNode;
...
interface IncrementalRowPlan extends IncrementalRowCandidate {
...
interface DeviceFeedbackDescriptor {
...
function webGpuTypedNearestParameters(node) {
  ...
  for (const key of ['coordinate_transformation_mode', 'coordinate_transform_mode']) {
    if (params[key] != null && params[key] !== 'asymmetric') {
      throw new Error(`WebGPU ${node.opType} node ${node.id} supports raw I8/U8 nearest resize only with ${key} "asymmetric".`);
    }
  }
...
const WEBGPU_PACKED_DOT4_FEATURE = 'packed_4x8_integer_dot_product';

function webGpuHasPackedDot4(languageFeatures) {
  return languageFeatures?.has?.(WEBGPU_PACKED_DOT4_FEATURE) === true;
}
...
function webGpuSliceDescriptor(node, input, output) {
  ...
  if (!input || !output || input.dtype !== "float32" || output.dtype !== "float32" ||
    ...
    throw new Error(`WebGPU Slice node ${node.id} requires rank-1..8 F32 input/output tensors.`);
  }
...
export class GraphExecutor extends BackendEngine {
  // GraphExecutor is a graph-bound scheduler composed by WebGPUEngine, not an
  // independently allocatable backend. This type-only member satisfies the
  // abstract base declaration without advertising a callable runtime method.
  ...
  declare deviceFeedbackControlBuffer: GPUBuffer | null;
  declare deviceFeedbackSequenceLength: number;
  declare deviceFeedbackState: Readonly<DeviceFeedbackState> | null;
  ...
  constructor(device: GPUDevice, graph: Graph, {
    ...
    // Inference compiles standalone Dropout as a buffer alias with no shader
    // dispatch. Autograd opts into distinct outputs only when training starts.
    this.training = training === true;
    // packed_4x8_integer_dot_product is a WGSL language feature, not a
    // GPUDevice feature.  Only modules selected behind this browser-advertised
    // gate contain the `requires` directive or packed dot builtins.
    this.hasPackedDot4 = webGpuHasPackedDot4(wgslLanguageFeatures);
    this._incrementalCacheValid = false;
    if (shaderLibrary) ShaderLibrary = shaderLibrary;
    this.pipelines = [];
    // A pipeline's auto layout is determined entirely by its shader module,
    // entry point, and override constants. Bind groups remain node-owned, so
    // identical operators can safely share this device object.
    this.computePipelineCache = /* @__PURE__ */ new Map();
    // Optional WGSL feature support can be advertised by the browser while a
    // particular driver still rejects the specialized module. Remember that
    // deterministic rejection so later identical nodes go straight to the
    // portable pipeline instead of recompiling and warning once per node.
    this.rejectedSpecializedShaders = /* @__PURE__ */ new Set();
    this.gpuBuffers = /* @__PURE__ */ new Map();
    this.adapterTargetBuffers = /* @__PURE__ */ new Map();
    this.auxiliaryBuffers = /* @__PURE__ */ new Set();
    this.incrementalRowPlans = /* @__PURE__ */ new Map();
    this.incrementalRowCandidates = /* @__PURE__ */ new Map();
    this.incrementalRowCopyTensorNames = /* @__PURE__ */ new Set();
    this.deviceFeedbackControlBuffer = null;
    this.deviceFeedbackSequenceLength = 0;
  ...
  _prepareIncrementalRowNode(node: ExecutorNode, position: number): ExecutorNode {
    ...
    const shadow = (tensor: ExecutorTensor | undefined): ExecutorTensor | undefined => {
      ...
      if (ArrayBuffer.isView(tensor.buffer) && !(tensor.buffer instanceof DataView)) {
        ...
        value = Object.assign(Object.create(Object.getPrototypeOf(tensor)), tensor, {
          buffer: this._rowTypedStorage(tensor),
        });
      }
    ...
    const prepared = quantizedRowNode(storageNode, position) as ExecutorNode;
    const normalize = (
      ...
      const normalized = Object.assign(Object.create(Object.getPrototypeOf(rowTensor)), rowTensor, {
        buffer: this._rowTypedStorage(rowTensor, rowTensor.sizeBytes),
      });
  ...
  _incrementalRowCandidate(
    ...
    if (this.training || output?.shape?.[0] !== 1 || !Number.isInteger(output.shape?.[1]) ||
        output.shape[1] <= 1) return null;
    let rowNode;
    try {
      rowNode = this._prepareIncrementalRowNode(node, 1);
    } catch {
      return null;
    }
    ...
    const addScratch = (
      ...
      if (!rowTensor || !originalTensor || rowTensor === originalTensor || typeof capacity !== 'number' ||
          !Number.isSafeInteger(capacity) || capacity <= 0 || capacity % 4 !== 0 ||
          rowTensor.sizeBytes % 4 !== 0) return false;
      const offset = rowTensor._webgpuRow?.offsetStride;
      if (typeof offset !== 'number' || !Number.isSafeInteger(offset) || offset < 0 || offset % 4 !== 0 ||
    ...
    for (const [name, rowTensor] of Object.entries(rowNode.inputs) as
      ...
      if (node.opType === 'QSDPA' && name === 'mask') {
        ...
        const capacity = typeof keyWidth === 'number' && Number.isSafeInteger(keyWidth)
          ...
          : 0;
        if (!addScratch(rowTensor, originalTensor, scratchInputs, capacity)) return null;
        continue;
      }
      if (!addScratch(rowTensor, originalTensor, scratchInputs)) return null;
    }
    for (const [name, rowTensor] of Object.entries(rowNode.outputs) as
      Array<[string, ExecutorTensor]>) {
      const originalTensor = node.outputs?.[name];
      if (!rowTensor || rowTensor === originalTensor) continue;
      if (!addScratch(rowTensor, originalTensor, scratchOutputs)) return null;
    }
  ...
  _analyzeIncrementalRows(): void {
    ...
    this.incrementalRowCandidates.clear();
    this.incrementalRowCopyTensorNames.clear();
    if (this.training || !(this.graph?.tensors instanceof Map)) return;
    for (let nodeIndex = 0; nodeIndex < this.graph.nodes.length; nodeIndex++) {
      const candidate = this._incrementalRowCandidate(this.graph.nodes[nodeIndex], nodeIndex);
      if (!candidate) continue;
      this.incrementalRowCandidates.set(nodeIndex, candidate);
      for (const name of candidate.scratchCapacities.keys()) {
        this.incrementalRowCopyTensorNames.add(name);
      }
    }
  }
  ...
  async _compileIncrementalRowPipelines(
    ...
    for (const nodeIndex of nodeIndices) {
      if (this.incrementalRowPlans.has(nodeIndex)) continue;
      const candidate = this.incrementalRowCandidates.get(nodeIndex);
      if (!candidate) continue;
      const buffers = new Map(this.gpuBuffers);
  ...
  async compile(): Promise<void> {
    ...
    for (const buffer of new Set(this.gpuBuffers.values())) buffer.destroy?.();
    for (const buffers of this.adapterTargetBuffers.values()) {
      buffers.a?.destroy?.();
      buffers.b?.destroy?.();
    }
    for (const buffer of this.auxiliaryBuffers) buffer.destroy?.();
    this.pipelines = [];
    this.gpuBuffers.clear();
    this.adapterTargetBuffers.clear();
    ...
    this.deviceFeedbackSequenceLength = 0;
    this.deviceFeedbackState = null;
    // The linearF32/linearInt8 shaders read output-major [d_out,d_in] weights.
    // Transpose input-major [d_in,d_out] weights once before VRAM upload.
    this._dinWeights = new Map();
    for (const n of this.graph.nodes) {
      if ((n.opType === "MatMul" || n.opType === "Linear" || n.opType === "Gemm") && n.wLayout === "din" && n.inputs.weight &&
          !n.inputs.scale && !n.inputs.weight_scale) {
    ...
    if (!this.training) this._aliasInferenceDropoutBuffers();
    for (let nodeIndex = 0; nodeIndex < this.graph.nodes.length; nodeIndex++) {
      const node = this.graph.nodes[nodeIndex];
      if (!this.training && node.opType === "Dropout") continue;
      const start = this.pipelines.length;
      await this._buildNodePipeline(node);
      for (let i = start; i < this.pipelines.length; i++) this.pipelines[i].graphNodeIndex = nodeIndex;
    }
  ...
  async prepareForTraining(): Promise<void> {
    if (this.training) return;
    this.training = true;
    // Recompile an already-live inference executor so Dropout outputs no longer
    // alias their inputs. This cost exists only after training is requested.
    if (this.compiledTopologyRevision != null) await this.compile();
  }

  async _ensureAdapterPipeline(): Promise<GPUComputePipeline> {
    if (this.adapterPipeline) return this.adapterPipeline;
    if (!ShaderLibrary) ({ ShaderLibrary } = await import('./ShaderLibrary.js'));
    const module = this.device.createShaderModule({ code: ShaderLibrary.getLoRAApplyShader() });
    this.adapterPipeline = await this.device.createComputePipelineAsync({
      layout: "auto",
  ...
  _adapterBuffers(target: AdapterTarget): { a: GPUBuffer; b: GPUBuffer } {
    let buffers = this.adapterTargetBuffers.get(target);
    ...
    this.adapterTargetBuffers.set(target, buffers);
    return buffers;
  }
  ...
  _createAuxiliaryStorageBuffer(label: string, values: ArrayBufferView): GPUBuffer {
    ...
    this.auxiliaryBuffers.add(buffer);
    return buffer;
  }
  ...
  async _cachedComputePipeline(
    ...
    if (!pending) {
      ...
      try {
        pending = Promise.resolve(this.device.createComputePipelineAsync({ layout: "auto", compute }));
      } catch (error) {
  ...
  releaseAdapterTargets(targets: readonly AdapterTarget[] | null | undefined): void {
    ...
    for (const target of targets || []) {
      const buffers = this.adapterTargetBuffers.get(target);
      if (!buffers) continue;
      this.adapterTargetBuffers.delete(target);
      retired.push(buffers.a, buffers.b);
    }
    const destroy = () => { for (const buffer of retired) buffer?.destroy?.(); };
    if (retired.length && this.device.queue.onSubmittedWorkDone) {
  ...
  async _prepareAdapterDispatches(
    ...
    const pipeline = await this._ensureAdapterPipeline();
    for (let nodeIndex = 0; nodeIndex < this.graph.nodes.length; nodeIndex++) {
      const node = this.graph.nodes[nodeIndex];
      ...
      for (let routeIndex = 0; routeIndex < routes.length; routeIndex++) {
        ...
        const factors = this._adapterBuffers(target);
        const bindGroup = this.device.createBindGroup({
          layout: pipeline.getBindGroupLayout(0),
          entries: [
            { binding: 0, resource: { buffer: this._buffer(input) } },
            { binding: 1, resource: { buffer: factors.a } },
            { binding: 2, resource: { buffer: factors.b } },
            { binding: 3, resource: { buffer: this._buffer(output) } },
  ...
  _allocateBuffers(): void {
    for (const [name, tensor] of this.graph.tensors.entries()) {
      ...
      if (this.incrementalRowCopyTensorNames.has(name)) {
  ...
  _aliasInferenceDropoutBuffers(): void {
    for (const node of this.graph.nodes) {
      ...
      const output = node.outputs.out || Object.values(node.outputs || {})[0];
      const source = input && this.gpuBuffers.get(input.name);
      const allocated = output && this.gpuBuffers.get(output.name);
      ...
      this.gpuBuffers.set(output.name, source);
    }
  }
  async _buildNodePipeline(node: ExecutorNode, {
    pipelines = this.pipelines,
    buffers = this.gpuBuffers,
  }: {
    pipelines?: CompiledWebGPUPipeline[];
    buffers?: Map<string, GPUBuffer>;
  } = {}): Promise<void> {
    const previousPipelines = this._activePipelineTarget;
    const previousBuffers = this._activePipelineBuffers;
    this._activePipelineTarget = pipelines;
    this._activePipelineBuffers = buffers;
    try {
      return await this._buildNodePipelineImpl(node);
    } finally {
      this._activePipelineTarget = previousPipelines;
      this._activePipelineBuffers = previousBuffers;
    }
  }
  ...
  async _buildNodePipelineImpl(node: ExecutorNode): Promise<void> {
    ...
    if (node.opType === "QConv2D") {
      ...
      const scaleBuffer = this._createAuxiliaryStorageBuffer(`QConv2D_scales_${node.id}`, scales);
      const zeroPointBuffer = this._createAuxiliaryStorageBuffer(`QConv2D_zero_points_${node.id}`, zeroPoints);
      const biasBuffer = this._buffer(bias) || this._createAuxiliaryStorageBuffer(
      ...
      const advertisedWorkgroupLimit = this.device?.limits?.maxComputeWorkgroupsPerDimension;
      ...
      const tiledDotQConv = this.hasPackedDot4 && outputChannels >= 16 &&
      ...
      const paramsBuf = this.device.createBuffer({ size: 112, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      this.auxiliaryBuffers.add(paramsBuf);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(weight) } },
        { binding: 2, resource: { buffer: scaleBuffer } },
        { binding: 3, resource: { buffer: zeroPointBuffer } },
        { binding: 4, resource: { buffer: biasBuffer } },
        { binding: 5, resource: { buffer: this._buffer(output) } },
      ...
      const inputBuf = this._buffer(node.inputs.input);
      const weightBuf = this._buffer(node.inputs.weight);
      const outputBuf = this._buffer(node.outputs.out);
      ...
      const biasBuf = this._buffer(node.inputs.bias) || this.device.createBuffer({
      ...
      const shaderModule = this.device.createShaderModule({
        code: hasTypedStorage ? ShaderLibrary.getTypedConcatCopyShader() : ShaderLibrary.getConcatCopyShader(),
      });
      ...
      for (const [, t] of inputs) {
        ...
        const size = hasTypedStorage ? webGpuTypedElementCount(t) : t.sizeBytes / 4;
        if (size == null) throw new Error(`Concat node ${node.id} has invalid I8/U8 input storage.`);
        if (hasTypedStorage) {
        ...
        const paramsBuf = this.device.createBuffer({ size: p.byteLength, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
        this.device.queue.writeBuffer(paramsBuf, 0, p);
        const bindGroup = this.device.createBindGroup({
          layout: pipeline.getBindGroupLayout(0),
          entries: [
            { binding: 0, resource: { buffer: this._buffer(t) } },
            { binding: 1, resource: { buffer: this._buffer(node.outputs.out) } },
            { binding: 2, resource: { buffer: paramsBuf } }
          ]
        });
        (this._activePipelineTarget || this.pipelines).push({ pipeline, bindGroup, workgroupCount: [Math.ceil(size / 64), 1, 1], nodeName: `${node.id}_concat` });
      ...
      const scales = Float32Array.from(weightQuantization.scales, Math.fround);
      const zeroPoints = Int32Array.from(weightQuantization.zero_points);
      const scaleBuffer = this._createAuxiliaryStorageBuffer(`QLinear_scales_${node.id}`, scales);
      const zeroPointBuffer = this._createAuxiliaryStorageBuffer(`QLinear_zero_points_${node.id}`, zeroPoints);
      ...
      bindGroupEntries = [
        ...
        { binding: 2, resource: { buffer: scaleBuffer } },
        { binding: 3, resource: { buffer: zeroPointBuffer } },
        { binding: 4, resource: { buffer: this._buffer(bias) } },
        { binding: 5, resource: { buffer: this._buffer(output) } },
        { binding: 6, resource: { buffer: paramsBuf } },
      ];
      ...
      if (scale) {
        ...
        if (!['int8', 'uint8'].includes(weight.dtype) ||
            ...
            (zeroPoint && (zeroPointType < 1 || zeroPointElements == null ||
        ...
        const dummyZeroPoint = this.device.createBuffer({
          size: 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
        });
        this.device.queue.writeBuffer(dummyZeroPoint, 0, new Uint32Array(1));
        const p = new Uint32Array([
        ...
        bindGroupEntries = [
          { binding: 0, resource: { buffer: this._buffer(input) } },
          { binding: 1, resource: { buffer: this._buffer(weight) } },
          { binding: 2, resource: { buffer: this._buffer(scale) } },
          { binding: 3, resource: { buffer: zeroPoint ? this._buffer(zeroPoint) : dummyZeroPoint } },
          { binding: 4, resource: { buffer: bias ? this._buffer(bias) : dummyBias } },
          { binding: 5, resource: { buffer: this._buffer(output) } },
          { binding: 6, resource: { buffer: paramsBuf } },
        ];
      } else {
        ...
        bindGroupEntries = [
          { binding: 0, resource: { buffer: this._buffer(input) } },
          { binding: 1, resource: { buffer: this._buffer(weight) } },
          { binding: 3, resource: { buffer: bias ? this._buffer(bias) : dummyBias } },
          { binding: 4, resource: { buffer: this._buffer(output) } },
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.input) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.weight) } },
        { binding: 2, resource: { buffer: biasBuf } },
        { binding: 3, resource: { buffer: this._buffer(node.outputs.out) } },
      ...
      this.device.queue.writeBuffer(paramsBuf, 0, raw);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(weight) } },
        { binding: 2, resource: { buffer: this._buffer(bias) } },
        { binding: 3, resource: { buffer: this._buffer(output) } },
        { binding: 4, resource: { buffer: paramsBuf } },
      ];
      ...
      if (Object.keys(node.inputs || {}).length !== 2 || !input || !weight || !output || outputCount !== 1 ||
          input.isInput !== true || input.dtype !== 'int32' || input.shape.length < 1 ||
        ...
        throw new Error(`WebGPU QEmbedding node ${node.id} requires graph-input I32 IDs, rank-2 I8/U8 weights, and I8/U8 output storage.`);
      }
      ...
      const zeroPoints = Int32Array.from(weightQuantization.zero_points);
      const scaleBuffer = this._createAuxiliaryStorageBuffer(`QEmbedding_scales_${node.id}`, scales);
      const zeroPointBuffer = this._createAuxiliaryStorageBuffer(`QEmbedding_zero_points_${node.id}`, zeroPoints);
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.input) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.weight) } },
        { binding: 2, resource: { buffer: this._buffer(node.outputs.out) } },
      ...
      const paramsBuf = this.device.createBuffer({ size: 32, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      const dummyBias = this.device.createBuffer({ size: Math.max(4, numExperts * 4), usage: GPUBufferUsage.STORAGE });
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(weight) } },
        { binding: 2, resource: { buffer: bias ? this._buffer(bias) : dummyBias } },
        { binding: 3, resource: { buffer: this._buffer(indices) } },
        { binding: 4, resource: { buffer: this._buffer(weights) } },
        { binding: 5, resource: { buffer: paramsBuf } },
      ];
      ...
      const paramsBuf = this.device.createBuffer({ size: 32, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      const dummyBias = this.device.createBuffer({ size: Math.max(4, numExperts * dOut * 4), usage: GPUBufferUsage.STORAGE });
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(expertWeight) } },
        { binding: 2, resource: { buffer: expertBias ? this._buffer(expertBias) : dummyBias } },
        { binding: 3, resource: { buffer: this._buffer(routeIndices) } },
        { binding: 4, resource: { buffer: this._buffer(routeWeights) } },
        { binding: 5, resource: { buffer: this._buffer(output) } },
        { binding: 6, resource: { buffer: paramsBuf } },
      ];
      ...
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.qkv) } },
        { binding: 1, resource: { buffer: this._buffer(mask) || this._buffer(node.inputs.qkv) } },
        { binding: 2, resource: { buffer: this._buffer(node.outputs.out) } },
        { binding: 3, resource: { buffer: paramsBuf } }
      ];
      ...
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.q) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.k) } },
        { binding: 2, resource: { buffer: this._buffer(node.inputs.v) } },
        { binding: 3, resource: { buffer: this._buffer(mask) || this._buffer(node.inputs.q) } },
        { binding: 4, resource: { buffer: this._buffer(node.outputs.out) } },
        { binding: 5, resource: { buffer: paramsBuf } }
      ];
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(q) } },
        { binding: 1, resource: { buffer: this._buffer(kv) } },
        { binding: 2, resource: { buffer: this._buffer(weight) } },
        { binding: 3, resource: { buffer: scale ? this._buffer(scale) : dummyScale } },
        { binding: 4, resource: { buffer: bias ? this._buffer(bias) : dummyBias } },
        { binding: 5, resource: { buffer: this._buffer(output) } },
        { binding: 6, resource: { buffer: paramsBuf } }
      ];
      ...
      const descriptor = webGpuQGroupNormDescriptor(
        node, this._activePipelineBuffers || this.gpuBuffers,
      );
      ...
      this.auxiliaryBuffers.add(statsBuffer);
      ...
      this.device.queue.writeBuffer(paramsBuffer, 0, parameterData);
      this.auxiliaryBuffers.add(paramsBuffer);

      const statsModule = this.device.createShaderModule({ code: ShaderLibrary.getQGroupNormStatsShader() });
      const statsPipeline = await this.device.createComputePipelineAsync({
      ...
      const statsBindGroup = this.device.createBindGroup({
        ...
        entries: [
          { binding: 0, resource: { buffer: this._buffer(descriptor.input) } },
          { binding: 1, resource: { buffer: statsBuffer } },
          { binding: 2, resource: { buffer: paramsBuffer } },
        ],
      });

      const applyModule = this.device.createShaderModule({ code: ShaderLibrary.getQGroupNormApplyShader() });
      const applyPipeline = await this.device.createComputePipelineAsync({
      ...
      const applyBindGroup = this.device.createBindGroup({
        ...
        entries: [
          { binding: 0, resource: { buffer: this._buffer(descriptor.input) } },
          { binding: 1, resource: { buffer: this._buffer(descriptor.weight) } },
          { binding: 2, resource: { buffer: this._buffer(descriptor.bias) } },
          { binding: 3, resource: { buffer: statsBuffer } },
          { binding: 4, resource: { buffer: this._buffer(descriptor.output) } },
          { binding: 5, resource: { buffer: paramsBuffer } },
        ],
      });
      (this._activePipelineTarget || this.pipelines).push({
      ...
      const statsPipeline = await this._cachedComputePipeline(
      ...
      const applyPipeline = await this._cachedComputePipeline(
      ...
      this.device.queue.writeBuffer(paramsBuffer, 0, parameterData);
      this.auxiliaryBuffers.add(paramsBuffer);
      const module = this.device.createShaderModule({ code: ShaderLibrary.getQMaskedMeanShader() });
      const pipeline = await this.device.createComputePipelineAsync({
      ...
      const bindGroup = this.device.createBindGroup({
        ...
        entries: [
          { binding: 0, resource: { buffer: this._buffer(descriptor.input) } },
          { binding: 1, resource: { buffer: this._buffer(descriptor.mask) } },
          { binding: 2, resource: { buffer: this._buffer(descriptor.output) } },
      ...
      this.device.queue.writeBuffer(paramsBuffer, 0, parameterData);
      this.auxiliaryBuffers.add(paramsBuffer);
      const dummyMask = descriptor.mask ? null : this.device.createBuffer({
        label: `QSDPA_dummy_mask_${node.id}`,
        size: 4,
        usage: GPUBufferUsage.STORAGE,
      });
      if (dummyMask) this.auxiliaryBuffers.add(dummyMask);

      const pipeline = await this._cachedComputePipeline(ShaderLibrary.getQSDPAShader());
      const bindGroup = this.device.createBindGroup({
        layout: pipeline.getBindGroupLayout(0),
        entries: [
          { binding: 0, resource: { buffer: this._buffer(descriptor.q) } },
          { binding: 1, resource: { buffer: this._buffer(descriptor.k) } },
          { binding: 2, resource: { buffer: this._buffer(descriptor.v) } },
          { binding: 3, resource: { buffer: this._buffer(descriptor.mask) || dummyMask } },
          { binding: 4, resource: { buffer: this._buffer(descriptor.output) } },
          { binding: 5, resource: { buffer: paramsBuffer } },
        ],
      });
      ...
      this.auxiliaryBuffers.add(paramsBuffer);
      const pipeline = await this._cachedComputePipeline(ShaderLibrary.getQArgMaxShader());
      const bindGroup = this.device.createBindGroup({
        layout: pipeline.getBindGroupLayout(0),
        entries: [
          { binding: 0, resource: { buffer: this._buffer(descriptor.input) } },
          { binding: 1, resource: { buffer: this._buffer(descriptor.output) } },
          { binding: 2, resource: { buffer: paramsBuffer } },
        ],
      });
      (this._activePipelineTarget || this.pipelines).push({
      ...
      if (!input || !output || input === output ||
          (this._buffer(input) && this._buffer(input) === this._buffer(output)) ||
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(output) } },
        { binding: 2, resource: { buffer: paramsBuf } },
      ];
      workgroupCount = [Math.ceil(Math.ceil(outputElements / 4) / 64), 1, 1];
      ...
      workgroupCount = [hasTypedStorage ? Math.ceil(Math.ceil(elements / 4) / 64) : Math.ceil(elements / 64), 1, 1];
      ...
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(a) } },
        { binding: 1, resource: { buffer: this._buffer(b) } },
        { binding: 2, resource: { buffer: this._buffer(output) } },
        { binding: 3, resource: { buffer: paramsBuf } },
      ];
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.a) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.b) } },
        { binding: 2, resource: { buffer: this._buffer(node.outputs.out) } },
      ...
      const weightLength = node.inputs.weight.sizeBytes / 4;
      if (node.inputs.weight.dtype !== "float32" || (weightLength !== 1 && weightLength !== c)) {
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(node.inputs.input) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.weight) } },
        { binding: 2, resource: { buffer: this._buffer(node.inputs.bias) } },
        { binding: 3, resource: { buffer: this._buffer(node.inputs.running_mean) } },
        { binding: 4, resource: { buffer: this._buffer(node.inputs.running_var) } },
        { binding: 5, resource: { buffer: this._buffer(node.outputs.out) } },
        { binding: 6, resource: { buffer: paramsBuf } }
      ];
      ...
      for (let o = 0; o < numOutputs; o++) {
        ...
        const bindGroup = this.device.createBindGroup({
          ...
          entries: [
            { binding: 0, resource: { buffer: this._buffer(node.inputs.input) } },
            { binding: 1, resource: { buffer: this._buffer(outT) } },
            { binding: 2, resource: { buffer: paramsBuf } }
          ]
        });
        (this._activePipelineTarget || this.pipelines).push({
      ...
      if (node.inputs.min) minVal = float32Storage(node.inputs.min.buffer)[0];
      if (node.inputs.max) maxVal = float32Storage(node.inputs.max.buffer)[0];
      ...
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(boxes) } },
        { binding: 1, resource: { buffer: this._buffer(scores) } },
        { binding: 2, resource: { buffer: this._buffer(output) } },
        { binding: 3, resource: { buffer: paramsBuf } }
      ];
      ...
      if (!cond || !a || !b || !output ||
        ...
        throw new Error(`${node.opType} node ${node.id} requires exact-shape F32 operands/output and an F32 or I32 condition.`);
      }
      wgslCode = ShaderLibrary.getWhereTypedShader();
      const paramsBuf = this.device.createBuffer({ size: 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, new Uint32Array([
        outputElements, cond.dtype === 'float32' ? 0 : 1, 0, 0,
      ]));
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(cond) } },
        { binding: 1, resource: { buffer: this._buffer(a) } },
        { binding: 2, resource: { buffer: this._buffer(b) } },
        { binding: 3, resource: { buffer: this._buffer(output) } },
        { binding: 4, resource: { buffer: paramsBuf } }
      ];
      ...
      const p = new Uint32Array([
        ...
        zeroPoint ? webGpuTypedDtypeCode(zeroPoint.dtype) : 0,
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(scale) } },
        ...
        { binding: 2, resource: { buffer: zeroPoint ? this._buffer(zeroPoint) : this._buffer(input) } },
        { binding: 3, resource: { buffer: this._buffer(output) } },
        { binding: 4, resource: { buffer: paramsBuf } }
      ];
      workgroupCount = [Math.ceil(outputElements / 64), 1, 1];
      ...
      const outShape = node.outputs.out.shape;
      if (!inp || inp.shape.length === 0 || inp.shape.length > outShape.length || outShape.length > 8 ||
      ...
      p.set(inp.shape, 4);
      p.set(outShape, 12);
      const paramsBuf = this.device.createBuffer({ size: 80, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(inp) } },
        { binding: 1, resource: { buffer: this._buffer(node.outputs.out) } },
        { binding: 2, resource: { buffer: paramsBuf } }
      ];
      workgroupCount = [Math.ceil(elements / 64), 1, 1];
      ...
      const paramsBuf = this.device.createBuffer({ size: 144, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
      this.device.queue.writeBuffer(paramsBuf, 0, p);
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(input) } },
        { binding: 1, resource: { buffer: this._buffer(output) } },
      ...
      if (indices.dtype === "int32") {
        ...
        bindGroupEntries = [
          { binding: 0, resource: { buffer: this._buffer(inp) } },
          { binding: 1, resource: { buffer: this._buffer(indices) } },
          { binding: 2, resource: { buffer: this._buffer(output) } },
          { binding: 3, resource: { buffer: paramsBuf } }
        ];
      } else if (indices.dtype === "float32" && axis === 0) {
        // Retain the existing native-compatible browser path for legacy F32
        // axis-0 index tensors while all new/general paths use I32 indices.
        wgslCode = ShaderLibrary.getGatherShader();
        const rowSize = inp.shape.slice(1).reduce((product, dimension) => product * dimension, 1) || 1;
        const indexElements = indices.shape.reduce((product, dimension) => product * dimension, 1);
        const paramsBuf = this.device.createBuffer({ size: 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
        this.device.queue.writeBuffer(paramsBuf, 0, new Uint32Array([rowSize, indexElements, total]));
        bindGroupEntries = [
          { binding: 0, resource: { buffer: this._buffer(inp) } },
          { binding: 1, resource: { buffer: this._buffer(indices) } },
          { binding: 2, resource: { buffer: this._buffer(output) } },
        ...
        throw new Error(`Gather node ${node.id} requires I32 indices; legacy F32 indices are supported only for axis 0.`);
      }
      workgroupCount = [Math.ceil(total / 64), 1, 1];
      ...
      bindGroupEntries = [
        { binding: 0, resource: { buffer: this._buffer(inp) } },
        { binding: 1, resource: { buffer: this._buffer(node.inputs.weight) } },
        { binding: 2, resource: { buffer: hasBias ? this._buffer(node.inputs.bias) : dummyBias } },
        { binding: 3, resource: { buffer: this._buffer(node.outputs.out) } },
        { binding: 4, resource: { buffer: paramsBuf } }
      ];
    ...
    if (fallbackWgslCode && fallbackWgslCode !== wgslCode && fallbackWorkgroupCount &&
        this.rejectedSpecializedShaders.has(wgslCode)) {
      wgslCode = fallbackWgslCode;
      workgroupCount = fallbackWorkgroupCount;
    }
    let pipeline;
    try {
      pipeline = await this._cachedComputePipeline(wgslCode);
    } catch (err) {
      ...
      this.rejectedSpecializedShaders.add(wgslCode);
      wgslCode = fallbackWgslCode;
      workgroupCount = fallbackWorkgroupCount;
      pipeline = await this._cachedComputePipeline(wgslCode);
    }
    ...
    (this._activePipelineTarget || this.pipelines).push({ pipeline, bindGroup, workgroupCount, nodeName: node.id });
  }
  ...
  _preflightQEmbeddingIds(inputs: WebGPUExecutionInputs): void {
    for (const node of this.graph.nodes) {
      ...
      const supplied = Object.prototype.hasOwnProperty.call(inputs, ids?.name)
        ? inputs[ids.name]
        : null;
      if (!ids?.isInput || ids.dtype !== 'int32' || !weight || weight.shape?.length !== 2 || !supplied) {
  ...
  _incrementalRowRange(
    ...
    if (!Number.isSafeInteger(offset) || offset < 0 || offset % 4 !== 0 ||
      ...
      throw new Error(`WebGPU incremental row ${label} requires four-byte-aligned in-bounds storage.`);
    }
    return { offset, size };
  }
    ...
    for (const [name, data] of Object.entries(inputs)) {
      ...
      if (rowPosition != null) {
        ...
        if (sourceBytes % 4 !== 0 || destinationOffset % 4 !== 0) {
          throw new Error(`WebGPU incremental row input '${name}' requires four-byte-aligned rows.`);
        }
      }
      if (sourceBytes % 4 === 0) {
        ...
        // WebGPU writeBuffer requires a four-byte write size. Byte tensors are
        // physically allocated with the same alignment, so upload a zero-padded
        // copy without changing their logical Tensor byte length.
        const padded = new Uint8Array(Math.ceil(sourceBytes / 4) * 4);
        padded.set(new Uint8Array(data.buffer, data.byteOffset + sourceOffset, sourceBytes));
        this.device.queue.writeBuffer(buffer, destinationOffset, padded);
      }
    }
  }
  ...
  _encodeIncrementalRowsInto(
    ...
    for (const nodeIndex of selectedNodes) {
      ...
      for (const name of plan.scratchInputs) {
        const rowTensor = this._rowTensorByName(rowNode.inputs, name);
        const fullTensor = this.graph.tensors.get(name) as ExecutorTensor;
        const fullBuffer = this.gpuBuffers.get(name);
        const scratch = plan.scratchByName.get(name)!;
        const { offset, size } = this._incrementalRowRange(
      ...
      for (const name of plan.scratchOutputs) {
        const rowTensor = this._rowTensorByName(rowNode.outputs, name);
        const fullTensor = this.graph.tensors.get(name) as ExecutorTensor;
        const fullBuffer = this.gpuBuffers.get(name);
        const scratch = plan.scratchByName.get(name)!;
        const { offset, size } = this._incrementalRowRange(
  ...
  _encodeIncrementalRows(selectedNodes: Iterable<number>, rowPosition: number): void {
    ...
    this._encodeIncrementalRowsInto(commandEncoder, selectedNodes, rowPosition);
  ...
  _deviceFeedbackDescriptor(
    ...
    const tokenInput = this.graph?.tensors?.get(tokenInputName);
    const keepInput = this.graph?.tensors?.get(keepInputName);
    const output = this.graph?.tensors?.get(outputName);
    ...
    if (!output || output.dtype !== 'int32' || output.shape?.length !== 2 ||
        ...
        !this.graph.outputNames?.includes(outputName)) {
    ...
    const selectedNodesValue = incrementalNodeSelection(
      this.graph as Graph, inputs, { incremental: true, changedInputs }, true,
    );
    ...
    for (const nodeIndex of selectedNodes) {
      ...
      if (nodeOutput?.shape?.[0] !== 1 || nodeOutput.shape?.[1] !== sequenceLength ||
          !this.incrementalRowCandidates.has(nodeIndex)) {
  ...
  _deviceFeedbackControl(sequenceLength: number): GPUBuffer {
    if (this.deviceFeedbackControlBuffer) {
      if (this.deviceFeedbackSequenceLength !== sequenceLength) {
        throw new Error('WebGPU device-feedback control buffer does not match the compiled sequence.');
      }
      return this.deviceFeedbackControlBuffer;
    }
    ...
    this.auxiliaryBuffers.add(buffer);
    this.deviceFeedbackControlBuffer = buffer;
    this.deviceFeedbackSequenceLength = sequenceLength;
    return buffer;
  }
  ...
  async executeDeviceFeedbackDecode(
    ...
    this.graph.assertTopologyRevision?.(this.compiledTopologyRevision as number, 'WebGPU');
    if (this.training) {
      throw new Error('WebGPU device-feedback decode is unavailable after entering training mode.');
    }
    ...
    if (this.graph.adapters?.hasActive()) {
      throw new Error('WebGPU device-feedback decode is unavailable while an adapter is active.');
    }
    const descriptor = this._deviceFeedbackDescriptor(inputs, options);
    const resume = descriptor.startPosition > 0;
    if (resume) {
      const state = this.deviceFeedbackState;
      const matchingState = state &&
        ...
        state.cacheGeneration === this.decodeCacheGeneration;
      if (!matchingState || this._incrementalCacheValid !== true) {
        throw new Error('WebGPU device-feedback resume requires the next position from a valid device-feedback seed/cache.');
      }
    }
    await this._compileIncrementalRowPipelines(descriptor.selectedNodes);
    for (const nodeIndex of descriptor.selectedNodes) {
      if (!this.incrementalRowPlans.has(nodeIndex)) {
        const node = this.graph.nodes[nodeIndex];
    ...
    const controlBuffer = descriptor.endPosition > firstRowPosition
      ? this._deviceFeedbackControl(descriptor.sequenceLength)
      : null;
    if (!resume) {
    ...
    const outputBuffer = this.gpuBuffers.get(descriptor.outputName);

    const tokenBuffer = this.gpuBuffers.get(descriptor.tokenInputName);
    const keepBuffer = this.gpuBuffers.get(descriptor.keepInputName);
    ...
    if (descriptor.endPosition === firstRowPosition) {
      this.deviceFeedbackState = Object.freeze({
        ...
        cacheGeneration: this.decodeCacheGeneration,
      });
      return outputBuffer;
    }
    ...
    try {
      for (let position = firstRowPosition; position < descriptor.endPosition; position++) {
        ...
        this._encodeIncrementalRowsInto(
    ...
    this._incrementalCacheValid = true;
    this.deviceFeedbackState = Object.freeze({
      tokenInputName: descriptor.tokenInputName,
      keepInputName: descriptor.keepInputName,
      outputName: descriptor.outputName,
  ...
  async execute(
    ...
    this.graph.assertTopologyRevision?.(this.compiledTopologyRevision!, "WebGPU");
    this.deviceFeedbackState = null;
    ...
    const adapterPlan = (hasAdapterSelector || this.graph.adapters?.hasActive())
      ? this.graph.adapters._pinExecution(options)
      : null;
    const incremental = incrementalExecutionEnabled(options, adapterPlan, !this.training);
    const cacheWasValid = this._incrementalCacheValid;
    const selectedNodes = incremental
      ? incrementalNodeSelection(this.graph as Graph, inputs, options, cacheWasValid)
      : null;
    this._incrementalCacheValid = false;
    const rowPosition = incrementalRowPosition(options, selectedNodes, cacheWasValid);
    if (rowPosition != null) {
      const rowNodes = selectedNodes!;
      await this._compileIncrementalRowPipelines(rowNodes);
      for (const nodeIndex of rowNodes) {
        ...
        const sequence = (this.graph.nodes[nodeIndex].outputs?.out ||
          Object.values(this.graph.nodes[nodeIndex].outputs || {})[0])?.shape?.[1];
        if (typeof sequence !== 'number' || !Number.isInteger(sequence) || rowPosition >= sequence) {
          throw new Error(`WebGPU W8A8 incremental row node ${this.graph.nodes[nodeIndex].id} position ${rowPosition} is outside its fixed sequence.`);
        }
      }
    }
    const pipelineOverrides = options.training?.pipelineOverrides || null;
    if (pipelineOverrides != null && !(pipelineOverrides instanceof Map)) {
      throw new Error("WebGPU training pipelineOverrides must be a Map.");
    }
    if ((this.graph.weightRevision || 0) !== this.compiledWeightRevision) {
    ...
    const adapterDispatches = adapterPlan ? await this._prepareAdapterDispatches(adapterPlan) : null;
    const adapterResources: GPUBuffer[] = [];
    this._uploadExecutionInputs(inputs, rowPosition, options.changedInputs);
    if (rowPosition != null) {
      this._encodeIncrementalRows(selectedNodes!, rowPosition);
      this._incrementalCacheValid = true;
    ...
    for (let i = 0; i < this.pipelines.length; i++) {
      const compiledPipeline = this.pipelines[i];
      if (selectedNodes && !selectedNodes.has(compiledPipeline.graphNodeIndex!)) continue;
      const p = pipelineOverrides?.get(i) || compiledPipeline;
      ...
      dispatched++;
      const nextNodeIndex = i + 1 < this.pipelines.length ? this.pipelines[i + 1].graphNodeIndex : -1;
    ...
    if (adapterResources.length && this.device.queue.onSubmittedWorkDone) {
  ...
  /**
   * Read one logical byte range without mapping the rest of a device tensor.
   * The copy is widened to WebGPU's four-byte alignment and trimmed after map.
   */
  async readBufferRange(
    gpuBuffer: GPUBuffer,
    byteOffset: number,
    sizeBytes: number,
    dtype: RuntimeDType = 'float32',
    ...
    const commandEncoder = this.device.createCommandEncoder();
    commandEncoder.copyBufferToBuffer(gpuBuffer, copyOffset, stagingBuffer, 0, paddedSize);
    this.device.queue.submit([commandEncoder.finish()]);
    let mapped = false;
    try {
      ...
      // Tensor.dtypeBytes above makes this unreachable, but retain an explicit
      // guard if a future runtime dtype is added without a readback view.
